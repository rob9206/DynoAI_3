...
def make_dense_baseline_csv(path: Path, rows: int = 12000, fs_hz: int = 20) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        for _ in range(rows):
            ...
            rpm = (
                2500
                + 1500 * math.sin(2 * math.pi * t / 30)
                + 500 * math.sin(2 * math.pi * t / 7)
            )
            ...
            mapk = (
                65
                + 30 * math.sin(2 * math.pi * t / 25)
                + 15 * math.sin(2 * math.pi * t / 11)
            )
...
def make_sparse_baseline_csv(path: Path, rows: int = 3000, fs_hz: int = 20) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        ...
        for _ in range(rows):
            ...
            # Rare knock events
            knock = (
                1 if (int(t * fs_hz) % 2000 == 0 and mapk > 80 and rpm > 3800) else 0
            )
...
def main():
    ...
    dense_metrics = calculate_baseline_metrics(
        dense_results["manifest"], dense_results["runtime"]
    )
    ...
    sparse_metrics = calculate_baseline_metrics(
        sparse_results["manifest"], sparse_results["runtime"]
    )
