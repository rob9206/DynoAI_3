...
def calculate_stability(
    csv_path: Path, outdir: Path, kernel_args: List[str] = None
) -> float:
    """
    ...
    shuffled_manifest = json.load(
        open(shuffled_outdir / "manifest.json", "r", encoding="utf-8")
    )
...
def run_kernel_experiment(
    ...
    return {
        ...
        "coverage_percent": (
            stats.get("bins_covered", 0) / max(1, stats.get("bins_total", 0))
        )
        * 100,
        "ve_energy": ve_energy,
...
def compare_to_baseline(
    experiment_metrics: dict, baseline_metrics: dict, dataset_type: str
) -> dict:
    ...
    return {
        "runtime_delta": experiment_metrics["runtime_seconds"]
        - baseline.get("runtime_seconds", 0),
        "coverage_delta": experiment_metrics["coverage_percent"]
        - baseline.get("coverage_percent", 0),
        "ve_energy_delta": experiment_metrics["ve_energy"]
        - baseline.get("ve_energy", 0),
        "bins_covered_delta": experiment_metrics["bins_covered"]
        - baseline.get("bins_covered", 0),
    }
...
def main():
    ...
    dense_csv = (
        ROOT / "experiments" / "outputs" / "dense_baseline" / "dense_baseline.csv"
    )
    sparse_csv = (
        ROOT / "experiments" / "outputs" / "sparse_baseline" / "sparse_baseline.csv"
    )
    ...
    for kernel in kernels:
        ...
        for dataset_type, csv_path in [("dense", dense_csv), ("sparse", sparse_csv)]:
            ...
            outdir = (
                ROOT / "experiments" / "outputs" / f"{kernel['name']}_{dataset_type}"
            )
            outdir.mkdir(parents=True, exist_ok=True)

            try:
                # Run experiment
                metrics = run_kernel_experiment(
                    csv_path, outdir, kernel["name"], dataset_type, kernel["args"]
                )

                # Compare to baseline
                comparison = compare_to_baseline(
                    metrics, baseline_metrics, dataset_type
                )
    ...
    print("-" * 80)
    print(
        f"{'Kernel':<15} {'Dataset':<10} {'Runtime':<10} {'Coverage':<10} {'VE Energy':<12} {'Stability':<12}"
    )
    print("-" * 80)
    ...
    for exp in successful_experiments:
        ...
        print(
            f"{exp.get('kernel_name', 'unknown'):<15} {dataset:<10} {runtime:<10.3f} {coverage:<10.3f} {energy:<12.3f} {stability:<12.3f}"
        )
