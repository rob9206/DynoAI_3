...
class BalanceAnalysis:
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        high_severity = sum(1 for cell in self.imbalanced_cells if cell.severity() == "high")
        medium_severity = sum(1 for cell in self.imbalanced_cells if cell.severity() == "medium")
        low_severity = sum(1 for cell in self.imbalanced_cells if cell.severity() == "low")
        
        return {
            "cells_analyzed": self.cells_analyzed,
            "cells_imbalanced": self.cells_imbalanced,
            "imbalance_percentage": round(100 * self.cells_imbalanced / self.cells_analyzed, 1) if self.cells_analyzed > 0 else 0,
    ...
    for r_idx in range(len(RPM_BINS)):
        ...
        for k_idx in range(len(KPA_BINS)):
            ...
            if count >= min_samples:
                ...
                afr_cmd_row.append(afr_cmd_sums[r_idx][k_idx] / count if afr_cmd_sums[r_idx][k_idx] > 0 else 0.0)
            else:
    ...
    for r_idx in range(len(RPM_BINS)):
        for k_idx in range(len(KPA_BINS)):
            ...
            if abs_delta >= afr_threshold:
                imbalanced_cells.append(ImbalanceCell(
                    rpm_idx=r_idx,
                    kpa_idx=k_idx,
                    rpm=RPM_BINS[r_idx],
                    kpa=KPA_BINS[k_idx],
...
def calculate_correction_factors(
    ...
    for cell in analysis.imbalanced_cells:
        ...
        if mode == BalanceMode.EQUALIZE:
            ...
            front_correction = _afr_error_to_ve_correction(cell.front_afr - avg_afr)
            rear_correction = _afr_error_to_ve_correction(cell.rear_afr - avg_afr)
            
        elif mode == BalanceMode.MATCH_FRONT:
            # Adjust rear to match front
            front_correction = 0.0
            rear_correction = _afr_error_to_ve_correction(cell.delta)
            
        elif mode == BalanceMode.MATCH_REAR:
            # Adjust front to match rear
            front_correction = _afr_error_to_ve_correction(-cell.delta)
            rear_correction = 0.0
        
        # Apply safety clamping
        front_factors[r_idx][k_idx] = _clamp_correction(front_correction, max_correction_pct)
        rear_factors[r_idx][k_idx] = _clamp_correction(rear_correction, max_correction_pct)
...
def _afr_error_to_ve_correction(afr_error: float) -> float:
    """
    Convert AFR error to VE correction percentage.
    
    Rule of thumb: 1 AFR point ≈ 7% VE change
    - AFR too high (lean) → need more fuel → increase VE (+)
    - AFR too low (rich) → need less fuel → decrease VE (-)
...
def _clamp_correction(correction: float, max_pct: float) -> float:
    ...
    clamped = max(-MAX_ABSOLUTE_CORRECTION / 100.0, min(MAX_ABSOLUTE_CORRECTION / 100.0, clamped))
...
def generate_balance_report(
    ...
    front_corrections_applied = sum(1 for row in front_factors for val in row if val != 0.0)
    rear_corrections_applied = sum(1 for row in rear_factors for val in row if val != 0.0)
...
def process_cylinder_balancing(
    ...
    logger.info(f"Running cylinder balancing (mode={mode}, max_correction={max_correction_pct}%)")
    ...
    logger.info(f"Found {analysis.cells_imbalanced} imbalanced cells (max delta: {analysis.max_delta:.2f} AFR)")
    ...
    report = generate_balance_report(analysis, front_factors, rear_factors, balance_mode, input_file)
