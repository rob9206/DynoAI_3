...
def generate_dense_dyno_csv(
    ...
    with output_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for _ in range(rows):
            ...
            rpm_base = rpm_center + rpm_amplitude * math.sin(2 * math.pi * t / rpm_period)
            ...
            map_base = map_center + map_amplitude * math.sin(2 * math.pi * t / map_period)
            # Add third harmonic for intermediate load points
            map_harmonic = map_amplitude * 0.3 * math.sin(2 * math.pi * t / (map_period / 3))
            ...
            afr_error_f = 0.15 * math.sin(2 * math.pi * (t - 0.2) / 11.3) + rnd.gauss(0, 0.08)
            afr_meas_f = afr_target_f + afr_error_f
            
            # Rear cylinder: slightly more variation (common V-twin characteristic)
            afr_error_r = 0.20 * math.sin(2 * math.pi * (t - 0.25) / 12.1) + rnd.gauss(0, 0.10)
            ...
            if mapk > knock_threshold_map and rpm > knock_threshold_rpm:
                knock_prob = 0.02 * ((mapk - knock_threshold_map) / 10) * ((rpm - knock_threshold_rpm) / 1000)
    ...
    print(f"     Duration: {rows/fs_hz:.1f} seconds ({rows/fs_hz/60:.1f} minutes)")
