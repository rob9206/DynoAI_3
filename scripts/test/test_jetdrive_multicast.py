        ...
        while time.time() - start_time < timeout:
            try:
                ...
                if len(data) >= 12:
                    ...
                    print(
                        f"  [OK] Packet #{packet_count} from {addr[0]}:{addr[1]} - "
                        f"Key={key}, Host={host}, Len={len(data)} bytes"
                    )
                else:
                    print(
                        f"  [OK] Packet #{packet_count} from {addr[0]}:{addr[1]} - {len(data)} bytes"
                    )
...
def main():
    ...
    if not any_success:
        ...
        print(
            "  4. Try selecting a different interface in Dynoware RT JetDrive settings"
        )
