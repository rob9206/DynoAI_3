...
class TrainingDataValidator:
    ...
    def _validate_session_physics(self, session: Dict[str, Any], idx: int) -> None:
        ...
        try:
            ...
            if all(
                k in build_data for k in ["bore_in", "stroke_in", "displacement_ci"]
            ):
                import math

                calculated_ci = (
                    (build_data["bore_in"] ** 2)
                    * math.pi
                    * build_data["stroke_in"]
                    * 2
                    / 4
                )
            ...
            if "cam_spec" in build_data:
                ...
                if "overlap_deg_front" in cam and "overlap_deg_rear" in cam:
                    avg_overlap = (
                        cam["overlap_deg_front"] + cam["overlap_deg_rear"]
                    ) / 2
                    if avg_overlap > 70.0:
                        self.errors.append(
    ...
    def _calculate_stats(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for session in sessions:
            ...
            if "build_config" in session:
                ...
                stats["engine_families"][engine] = (
                    stats["engine_families"].get(engine, 0) + 1
                )
    ...
    def print_report(
        self, file_path: Path, is_valid: bool, stats: Dict[str, Any]
    ) -> None:
...
Examples:
  ...
  python scripts/validate_training_data.py data.json --verbose
    ...
    parser.add_argument(
        "files", nargs="+", help="Training data JSON file(s) to validate"
    )
    ...
    validator = TrainingDataValidator(
        verbose=args.verbose, enable_physics=not args.no_physics
    )
