...
async def discover_with_status(
    config: JetDriveConfig, timeout: float = 5.0
) -> list[JetDriveProviderInfo]:
    """Discover providers with progress output."""
    print_info(
        f"Scanning for JetDrive providers on {config.multicast_group}:{config.port}..."
    )
    ...
    try:
        while True:
            ...
            if len(providers) != last_provider_count:
                if providers:
                    print(
                        f"[{timestamp}] {Colors.GREEN}Connected{Colors.RESET}: {len(providers)} provider(s)"
                    )
                    for p in providers:
...
Environment Variables:
  ...
  JETDRIVE_IFACE        Network interface IP (default: 0.0.0.0)
    ...
    parser.add_argument(
        "--diagnose", "-d", action="store_true", help="Run full system diagnostics"
    )
    parser.add_argument(
        "--discover",
        action="store_true",
    ...
    parser.add_argument(
        "--capture", "-c", action="store_true", help="Capture live data from JetDrive"
    )
    parser.add_argument(
        "--monitor",
        "-m",
