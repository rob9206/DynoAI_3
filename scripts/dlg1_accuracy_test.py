...
from api.services.innovate_client import InnovateClient, InnovateDeviceType, InnovateSample
...
def analyze_channel(stats: ChannelStats, channel: int) -> Dict:
    ...
    if len(stats.timestamps) > 1:
        ...
        results["sample_rate_hz"] = round(len(stats.samples) / duration, 2) if duration > 0 else 0
        
        # Inter-sample timing
        for i in range(1, len(stats.timestamps)):
            stats.inter_sample_times.append(stats.timestamps[i] - stats.timestamps[i-1])
        
        if stats.inter_sample_times:
            results["avg_sample_interval_ms"] = round(statistics.mean(stats.inter_sample_times) * 1000, 2)
            results["min_sample_interval_ms"] = round(min(stats.inter_sample_times) * 1000, 2)
            results["max_sample_interval_ms"] = round(max(stats.inter_sample_times) * 1000, 2)
            results["sample_interval_stdev_ms"] = round(statistics.stdev(stats.inter_sample_times) * 1000, 2) if len(stats.inter_sample_times) > 1 else 0
    ...
    if stats.afr_values:
        ...
        results["afr_stdev"] = round(statistics.stdev(stats.afr_values), 3) if len(stats.afr_values) > 1 else 0
...
def print_results(results: Dict, channel_name: str):
    ...
    print(f"       Min/Max: {results.get('min_sample_interval_ms', 0)} / {results.get('max_sample_interval_ms', 0)} ms")
    ...
    print(f"       Range: {results.get('lambda_min', 0)} - {results.get('lambda_max', 0)}")
...
""")
    ...
    try:
        ...
        if not issues:
            ...
            print(f"       - Sample rate: ~{(ch1_rate + ch2_rate) / 2:.1f} Hz per channel")
        ...
        for ch, afr, name in [(1, ch1_afr, "A"), (2, ch2_afr, "B")]:
            if 21.0 <= afr <= 23.0:
                print(f"  [OK] Sensor {name}: {afr:.1f} AFR - Calibrated (reading free air)")
            elif 20.0 <= afr <= 24.0:
                print(f"  [!!] Sensor {name}: {afr:.1f} AFR - Close, may need fine calibration")
            elif afr > 0:
                print(f"  [i] Sensor {name}: {afr:.1f} AFR - Not in free air (or needs calibration)")
            else:
