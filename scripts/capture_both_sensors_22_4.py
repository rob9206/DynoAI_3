...
if packets:
    ...
    for i in range(len(first_packet) - 1):
        ...
        if val_be == 224:
            print(f"  FOUND at byte {i}: 0x{first_packet[i]:02x}{first_packet[i+1]:02x} (big-endian)")
        if val_le == 224:
            print(f"  FOUND at byte {i}: 0x{first_packet[i]:02x}{first_packet[i+1]:02x} (little-endian)")
