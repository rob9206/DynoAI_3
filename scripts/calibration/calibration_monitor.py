...
""")
    ...
    try:
        ...
        if latest[1]:
            print(
                f"  Final CH1/A: {latest[1].afr:.1f} AFR (λ={latest[1].lambda_value:.3f})"
            )
        if latest[2]:
            print(
                f"  Final CH2/B: {latest[2].afr:.1f} AFR (λ={latest[2].lambda_value:.3f})"
            )
        ...
        for ch, name in [(1, "A"), (2, "B")]:
            if latest[ch]:
                ...
                if 21.0 <= afr <= 23.0:
                    print(
                        f"  Sensor {name}: {Colors.GREEN}✓ CALIBRATED (reading free air correctly){Colors.RESET}"
                    )
                elif afr >= 20.0:
                    print(
                        f"  Sensor {name}: {Colors.YELLOW}~ Close - may need fine calibration{Colors.RESET}"
                    )
                else:
                    print(
                        f"  Sensor {name}: {Colors.RED}✗ Not in free air or needs calibration{Colors.RESET}"
                    )
