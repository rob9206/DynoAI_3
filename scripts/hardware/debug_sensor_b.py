...
def decode_mts_packet_debug(data: bytes):
    ...
    print(
        f"    Current code (bytes [2],[3]): raw={raw_current}, lambda={lambda_current:.3f}, AFR={afr_current:.1f}"
    )
    ...
    print(
        f"    Try bytes [0],[1]:             raw={raw_01}, lambda={lambda_01:.3f}, AFR={afr_01:.1f}"
    )
    ...
    print(
        f"    Try bytes [1],[2]:             raw={raw_12}, lambda={lambda_12:.3f}, AFR={afr_12:.1f}"
    )
        ...
        while packet_count < 10:  # Capture 10 packets for analysis
            ...
            while len(buffer) >= 10:
                ...
                if result:
                    ...
                    print(
                        f"  Sensor B AFR (current):  {result['sensor_b_afr_current']:.1f}"
                    )
                    print(
                        f"  Sensor B AFR (alt 1):    {result['sensor_b_afr_alt1']:.1f}"
                    )
                    print(
                        f"  Sensor B AFR (alt 2):    {result['sensor_b_afr_alt2']:.1f}"
                    )
