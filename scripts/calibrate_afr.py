...
def calculate_offset(measured_afr: float, expected_afr: float = EXPECTED_FREE_AIR_AFR) -> float:
    """
...
""")
    ...
    input(f"{Colors.BOLD}Press ENTER when sensors are in free air and warmed up...{Colors.RESET} ")
    ...
    try:
        ...
        print(f"{Colors.BOLD}Collecting {SAMPLE_COUNT} samples from each sensor...{Colors.RESET}")
        ...
        for channel in [1, 2]:
            ...
            print(f"{Colors.BOLD}Channel {channel} (Sensor {'A' if channel == 1 else 'B'}):{Colors.RESET}")
            print(f"  Measured AFR: {status_color}{avg_afr:.2f} ± {std_afr:.2f}{Colors.RESET} (min: {min_afr:.2f}, max: {max_afr:.2f})")
            print(f"  Expected AFR: {Colors.GREEN}{EXPECTED_FREE_AIR_AFR}{Colors.RESET}")
            print(f"  Error: {status_color}{avg_afr - EXPECTED_FREE_AIR_AFR:+.2f} AFR{Colors.RESET}")
            print(f"  Status: {status_color}{status}{Colors.RESET}")
            
            if not is_calibrated(avg_afr):
                print(f"  {Colors.YELLOW}Recommended offset: {offset:+.2f} AFR{Colors.RESET}")
            else:
        ...
        if not needs_calibration:
            print(f"{Colors.GREEN}{Colors.BOLD}🎉 All sensors are properly calibrated!{Colors.RESET}")
        ...
        response = input(f"{Colors.BOLD}Apply calibration offsets to config file? (y/n):{Colors.RESET} ").lower()
        ...
        if not cal_file.exists():
            print(f"{Colors.RED}[ERROR] Calibration file not found: {cal_file}{Colors.RESET}")
            return 1
        ...
        for channel, data in results.items():
            ...
            if ch_str in cal_data["channels"]:
                ...
                cal_data["channels"][ch_str]["last_free_air_reading"] = round(data["average"], 2)
                cal_data["channels"][ch_str]["calibration_date"] = datetime.now().isoformat()
