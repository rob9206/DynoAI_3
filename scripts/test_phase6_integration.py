...
def test_unit_inference():
    ...
    assert confidence.confidence >= 0.5, "Unit match should give at least 0.5 confidence"
    assert any("Unit match" in r for r in confidence.reasons), "Should mention unit match"
...
def test_auto_detect():
    ...
    for name, conf in mappings.items():
        print(f"  {name}: {conf.source_name} (confidence: {conf.confidence:.2f})")
...
def test_low_confidence_detection():
    ...
    confidence_map = {
        ...
        "afr_front": MappingConfidence("afr_front", 15, "AFR", 0.6, [], ["Low confidence"]),
    }
    ...
    assert low_conf[0].canonical_name == "afr_front", "Should flag AFR as low confidence"
