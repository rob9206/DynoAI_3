@echo off
REM DynoAI Comprehensive Service Startup Script for Windows - VERBOSE VERSION
...
echo NOTE: This step can take several minutes depending on your internet connection.
echo.

REM Update Python pip with verbose output
echo [2a/10] Updating pip...
echo Command: %PYTHON_EXE% -m pip install --upgrade pip
%PYTHON_EXE% -m pip install --upgrade pip
if errorlevel 1 (
    echo WARNING: pip update failed or was skipped
) else (
    echo SUCCESS: pip updated
)

echo.
echo [2b/10] Updating Python packages from requirements.txt...
echo This may take several minutes...
echo Command: %PYTHON_EXE% -m pip install -U -r "%PROJECT_ROOT%\requirements.txt"
%PYTHON_EXE% -m pip install -U -r "%PROJECT_ROOT%\requirements.txt"
if errorlevel 1 (
    echo WARNING: Some Python packages failed to update
) else (
    echo SUCCESS: Python packages updated
)

echo.
echo [2c/10] Updating Node.js dependencies...
echo This may take several minutes...
cd "%PROJECT_ROOT%\frontend"
echo Current directory: %CD%
echo Command: call "%NPM_EXE%" update
call "%NPM_EXE%" update
if errorlevel 1 (
    echo WARNING: npm update failed or was skipped
) else (
