...
def main():
    ...
    print(
        f"  MAP range: {constraints.min_map_kpa}-{constraints.max_map_kpa} kPa"
    )
    ...
    print(
        f"  New coverage: {summary2['coverage_pct']:.1f}% (was {summary1['coverage_pct']:.1f}%)"
    )
    print(
        f"  Improvement: +{summary2['coverage_pct'] - summary1['coverage_pct']:.1f}%"
    )
    ...
    for name, rpm, map_range, test_type, priority in test_cases:
        ...
        gain, efficiency = score_test_efficiency(step,
                                                 summary2["coverage_pct"])

        print(
            f"  {name:25} | {gain:6.1f}%       | {efficiency:6.2f}    | P{priority}"
        )
