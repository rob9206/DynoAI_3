                ...
                if len(data) >= 2:
                    ...
                    for i in range(len(data) - 1):
                        ...
                        if 20.0 <= afr_be <= 25.0:  # Looking for ~22.4
                            print(f"  Possible AFR @ byte {i}: {afr_be:.1f} (big-endian)")
                        ...
                        afr_le = val_le / 10.0
                        if 20.0 <= afr_le <= 25.0:
                            print(f"  Possible AFR @ byte {i}: {afr_le:.1f} (little-endian)")
                ...
                if len(data) >= 4:
                    for i in range(len(data) - 3):
                        try:
                            ...
                            if 20.0 <= afr_f <= 25.0:
                                print(f"  Possible AFR @ byte {i}: {afr_f:.1f} (float BE)")
                        ...
                        try:
                            afr_f = struct.unpack('<f', data[i:i+4])[0]
                            if 20.0 <= afr_f <= 25.0:
                                print(f"  Possible AFR @ byte {i}: {afr_f:.1f} (float LE)")
                                afr_readings.append(afr_f)
                        except:
                            pass
            else:
