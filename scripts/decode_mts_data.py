...
def live_decode(port: str = "COM5", baudrate: int = 19200, duration: float = 10.0):
    ...
    try:
                ...
                while len(buffer) >= 10:
                    ...
                    if len(buffer) >= 10:
                        ...
                        if result:
                            ...
                            print(
                                f"[{readings[-1]['time']:5.1f}s] CH1: λ={lambda1:.3f} AFR={afr1:5.1f} | CH2: λ={lambda2:.3f} AFR={afr2:5.1f}"
                            )
        ...
        print(f"\n{'=' * 70}")
        print(
            f"Summary: {len(readings)} readings in {duration}s ({len(readings) / duration:.1f} Hz)"
        )
        ...
        if readings:
            ...
            print(
                f"Channel 2 AFR: min={min(ch2_afrs):.1f}, max={max(ch2_afrs):.1f}, avg={sum(ch2_afrs) / len(ch2_afrs):.1f}"
            )
