...
def test_ai_correction_system():
    ...
    try:
        ...
        print(f"  [OK] Grid: {len(config['rpm_bins'])} RPM x {len(config['map_bins'])} MAP bins")
    ...
    for i in range(num_pulls):
        try:
            ...
            print(f"  Pull #{i+1}: Simulating at {rpm:.0f} RPM @ {map_kpa:.0f} kPa...", end=" ")
    ...
    try:
        ...
        if uncertainty:
            ...
            print(f"  [OK] Uncertainty range: {unc_array.min():.3f} - {unc_array.max():.3f}")
    ...
    try:
        ...
        if real_obs:
            ...
            if 50.0 <= min_ve <= max_ve <= 150.0:
                ...
                print(f"  [WARN] VE values outside expected range (might be deltas instead of absolute)")
    ...
    try:
        ...
        print(f"  [FAIL] Failed: {e}")
        print("  Note: This might fail if no corrections are cached. Run simulate() first.")
    ...
    print(f"[OK] Total observations: {sum(r['observations_added'] for r in pull_results)}")
