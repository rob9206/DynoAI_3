...
if (Test-Path .\fix_helppercents.py) {
  python .\fix_helppercents.py .\tools\ai_tuner_toolkit_dyno_v1_2.py
}
if (Test-Path .\patch_selftest.py) {
  python .\patch_selftest.py .\tools\ai_tuner_toolkit_dyno_v1_2.py
}

# 3) Run self-test
python .\tools\ai_tuner_toolkit_dyno_v1_2.py --selftest --outdir .\outputs
...
$cmd = "python .\tools\ai_tuner_toolkit_dyno_v1_2.py --csv `"$csv`" --outdir `"$out`" --smooth_passes 2 --clamp 15 --rear_bias 2.5 --rear_rule_deg 2.0 --hot_extra -1.0"
