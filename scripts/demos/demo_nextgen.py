...
def main():
    ...
    try:
        ...
        if result['success']:
            ...
            payload_path = Path(__file__).parent / "runs" / run_id / "NextGenAnalysis.json"
            if payload_path.exists():
                ...
                print(f"\n      Quick Stats:")
                print(f"        - Total samples: {data.get('inputs_present', {}).get('row_count', 'N/A')}")
                print(f"        - Surfaces: {len(data.get('surfaces', {}))}")
                print(f"        - Spark valleys found: {len(data.get('spark_valley', []))}")
                print(f"        - Hypotheses: {len(data.get('cause_tree', {}).get('hypotheses', []))}")
                print(f"        - Test plan steps: {len(data.get('next_tests', []))}")
                print(f"        - Coverage gaps: {len(data.get('coverage_gaps', []))}")
        else:
