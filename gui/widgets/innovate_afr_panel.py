...
from PyQt6.QtWidgets import (
    ...
    QGridLayout, QPushButton, QComboBox, QSlider, QSizePolicy
)
...
class AFRGaugeWidget(QWidget):
    ...
    def paintEvent(self, event) -> None:
        ...
        # Draw value arc
        color = QColor(get_afr_color(self._value, self._target) if self._connected else "#6b7280")
        pen = QPen(color, 8)
        pen.setCapStyle(Qt.PenCapStyle.RoundCap)
        painter.setPen(pen)
        ...
        # Draw target marker
        if self._connected:
            target_normalized = max(0, min(1, (self._target - min_afr) / (max_afr - min_afr)))
...
class InnovateAFRPanel(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self.target_value_label.setStyleSheet("font-weight: bold; font-family: monospace; min-width: 40px;")
