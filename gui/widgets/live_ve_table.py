...
ENGINE_PRESETS = {
    ...
    EnginePreset.SPORTBIKE_600: EngineConfig(
        ...
        rpm_bins=[2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000, 11000, 12000, 13000, 14000, 15000],
        map_bins=[20, 30, 40, 50, 60, 70, 80, 90, 100, 110],
        max_rpm=15000,
    ),
    EnginePreset.SPORTBIKE_1000: EngineConfig(
        name="Sportbike 1000cc",
        rpm_bins=[2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000, 11000, 12000, 13000],
        map_bins=[20, 30, 40, 50, 60, 70, 80, 90, 100, 110],
        max_rpm=13000,
    ),
...
class LiveVETable(QWidget):
    ...
    def __init__(
        ...
        # Active cells
        self._active_cells: List[Tuple[int, int, float]] = []  # (rpm_idx, map_idx, weight)
    ...
    def _build_ui(self) -> None:
        ...
        self.target_label.setStyleSheet(f"font-family: monospace; color: {COLORS['muted_foreground']};")
    ...
    def _calculate_active_cells(self) -> None:
        ...
        rpm_weight = 0
        if rpm_high != rpm_low:
            rpm_weight = max(0, min(1, (self._current_rpm - rpm_low) / (rpm_high - rpm_low)))
            
        map_weight = 0
        if map_high != map_low:
            map_weight = max(0, min(1, (self._current_map - map_low) / (map_high - map_low)))
