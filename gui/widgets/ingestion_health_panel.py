...
class ChannelHealthRow(QWidget):
    ...
    def __init__(self, channel: ChannelHealth, parent: Optional[QWidget] = None):
        ...
        icon.setStyleSheet(
            f"color: {get_health_color(channel.health)}; font-size: 10pt;"
        )
        ...
        rate_color = (
            "#22c55e"
            if channel.samples_per_second > 10
            else "#eab308"
            if channel.samples_per_second > 0
            else "#6b7280"
        )
        ...
        value.setStyleSheet(
            "font-family: monospace; font-size: 9pt; min-width: 50px; text-align: right;"
        )
...
class CircuitBreakerCard(QFrame):
    ...
    def __init__(
        self, name: str, breaker: CircuitBreakerState, parent: Optional[QWidget] = None
    ):
        super().__init__(parent)
        ...
        state_styles = {
            "closed": (
                "background-color: #22c55e20; border: 1px solid #22c55e30;",
                "#22c55e",
                "🛡",
            ),
            "open": (
                "background-color: #ef444420; border: 1px solid #ef444430;",
                "#ef4444",
                "📴",
            ),
            "half_open": (
                "background-color: #eab30820; border: 1px solid #eab30830;",
                "#eab308",
                "📶",
            ),
        }
        ...
        stats_label = QLabel(
            f"{breaker.failure_count} failures • {breaker.success_rate * 100:.0f}% success"
        )
...
class IngestionHealthPanel(QWidget):
    ...
    def _update_health_display(self, health: str) -> None:
        ...
        # Update stats
        healthy_count = sum(
            1 for ch in self._channels.values() if ch.health == "healthy"
        )
        ...
        drop_severity = (
            "critical"
            if self._frame_stats.drop_rate > 5
            else "warning"
            if self._frame_stats.drop_rate > 1
            else "healthy"
        )
        self._update_stat_value(
            self.frames_stat,
        ...
        if total_count > 0:
            ...
            sorted_channels = sorted(
                self._channels.values(), key=lambda ch: health_order.get(ch.health, 5)
            )
