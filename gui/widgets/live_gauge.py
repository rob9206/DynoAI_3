...
class NeedleGauge(QWidget):
    ...
    def paintEvent(self, event: QPaintEvent) -> None:
        ...
        percentage = (
            (self._value - self.config.min_val) / range_val if range_val > 0 else 0
        )
        ...
        painter.setPen(
            QPen(self.COLOR_ARC_BG, 6, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap)
        )
        ...
        painter.setPen(
            QPen(display_color, 6, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap)
        )
        ...
        for i in range(segments + 1):
            ...
            if self.config.warning:
                warning_segment = (
                    (self.config.warning - self.config.min_val) / range_val * segments
                )
                if i >= warning_segment:
                    tick_color = self.COLOR_WARNING
            if self.config.critical:
                critical_segment = (
                    (self.config.critical - self.config.min_val) / range_val * segments
                )
                if i >= critical_segment:
                    tick_color = self.COLOR_CRITICAL

            painter.setPen(
                QPen(tick_color, 2, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap)
            )
...
class CompactGauge(QWidget):
    ...
    def paintEvent(self, event: QPaintEvent) -> None:
        ...
        painter.drawText(
            QRectF(8, 4, width - 16, 16), Qt.AlignmentFlag.AlignLeft, self.label
        )
        ...
        painter.drawText(
            QRectF(8, 18, width - 16, 22), Qt.AlignmentFlag.AlignLeft, value_text
        )
