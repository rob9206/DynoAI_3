...
from gui.api.jetdrive_client import JetDriveClient, ConnectionStatus, JetDriveSample, RunInfo
from gui.components.card import Card, CardHeader, CardContent, CardTitle
from gui.components.button import Button, ButtonVariant, ButtonSize
from gui.widgets.live_gauge import NeedleGauge, GaugeConfig, CompactGauge
from gui.widgets.live_ve_table import LiveVETable, EnginePreset
...
class JetDrivePage(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        # RPM Gauge
        self.rpm_gauge = NeedleGauge(GaugeConfig(
            label="RPM",
            units="rpm",
            min_val=0,
            max_val=7000,
        ...
        # HP Gauge
        self.hp_gauge = NeedleGauge(GaugeConfig(
            label="Horsepower",
            units="HP",
            min_val=0,
            max_val=200,
        ...
        # Torque Gauge
        self.torque_gauge = NeedleGauge(GaugeConfig(
            label="Torque",
            units="ft-lb",
            min_val=0,
            max_val=200,
        ...
        # AFR Gauge
        self.afr_gauge = NeedleGauge(GaugeConfig(
            label="AFR",
            units=":1",
            min_val=10,
            max_val=18,
        ...
        self.runs_count_label.setStyleSheet(f"color: {COLORS['muted_foreground']}; font-size: 9pt;")
        ...
        self.no_runs_label.setStyleSheet(f"color: {COLORS['muted_foreground']}; padding: 24px;")
    ...
    def _on_run_completed(self, run: RunInfo) -> None:
        ...
        self.status_label.setText(f"✅ Run complete! Peak: {run.peak_hp:.1f} HP / {run.peak_tq:.1f} ft-lb")
    ...
    def _update_runs_list(self) -> None:
        ...
        self.runs_count_label.setText(f"{len(self._runs)} run{'s' if len(self._runs) != 1 else ''}")
        ...
        for run in self._runs[:5]:
            ...
            id_label.setStyleSheet(f"color: {COLORS['muted_foreground']}; font-size: 9pt;")
    ...
    def _on_dyno_config_loaded(self, config) -> None:
        ...
        if config:
            self.status_label.setText(f"🔧 Dyno: {config.model} (SN: {config.serial_number})")
