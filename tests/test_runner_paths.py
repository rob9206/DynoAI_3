...
def test_traversal_rejected(tmp_path: Path):
    ...
    assert "escapes repo root" in (r.stderr + r.stdout), "Expected 'escapes repo root' error message"
...
def test_invalid_idea_id():
    ...
    assert "Unknown idea-id" in (r.stderr + r.stdout), "Expected 'Unknown idea-id' error"
