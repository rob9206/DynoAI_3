...
class TestNextGenGenerateEndpoint:
    ...
    def test_generate_returns_200_and_creates_files(self, client,
                                                    nextgen_run_dir):
        ...
        with patch("api.routes.nextgen.get_nextgen_workflow"
            ...
            def mock_generate(rid, force=False):
                ...
                (output_dir / "NextGenAnalysis.json").write_text(
                    json.dumps(payload))
                (output_dir / "NextGenAnalysis_Meta.json").write_text(
                    json.dumps({"schema_version": "dynoai.nextgen@1"}))
    ...
    def test_generate_with_force_regenerates(self, client, nextgen_run_dir):
        ...
        with patch("api.routes.nextgen.get_nextgen_workflow"
            ...
            response2 = client.post(
                f"/api/nextgen/{run_id}/generate?force=true")
    ...
    def test_generate_with_include_full_returns_payload(
            self, client, nextgen_run_dir):
        ...
        with patch("api.routes.nextgen.get_nextgen_workflow"
            ...
            response = client.post(
                f"/api/nextgen/{run_id}/generate?include=full")
...
class TestNextGenGetEndpoint:
    ...
    def test_get_returns_payload_after_generation(self, client,
                                                  nextgen_run_dir):
...
class TestNextGenGracefulDegradation:
    ...
    def test_generate_succeeds_without_knock_column(self, client,
                                                    nextgen_run_no_knock):
    ...
    def test_knock_surfaces_omitted_when_missing(self, client,
                                                 nextgen_run_no_knock):
    ...
    def test_warning_recorded_for_missing_knock(self, client,
                                                nextgen_run_no_knock):
        ...
        with patch("api.routes.nextgen.get_nextgen_workflow"
            ...
            payload = {
                ...
                "surfaces": {},
                "notes_warnings":
                ["No knock data available; knock-related analysis disabled."],
            }
            ...
            knock_warning_found = any(
                "knock" in w.lower() or "No knock data" in w for w in warnings)
