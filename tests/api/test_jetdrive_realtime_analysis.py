...
class TestVEDelta:
    ...
    def test_ve_delta_stats(self, engine):
        ...
        engine.on_aggregated_sample(
            make_sample(rpm=2000.0, map_kpa=50.0, afr=13.0))

        # Lean sample
        engine.on_aggregated_sample(
            make_sample(rpm=4000.0, map_kpa=80.0, afr=16.0))
...
class TestAlertDetection:
    ...
    def test_implausible_afr_low_alert(self, engine):
        ...
        alerts = [
            a for a in engine.alerts if a.type == AlertType.IMPLAUSIBLE_AFR
        ]
        assert len(alerts) >= 1
    ...
    def test_missing_channel_alert(self):
        ...
        stale_alerts = [
            a for a in engine.alerts if a.type == AlertType.STALE_CHANNEL
        ]
