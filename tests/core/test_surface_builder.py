...
class TestSurfaceBuilderShapes:
    ...
    def sample_df(self):
        ...
        df = pd.DataFrame({
            "rpm": rpm_values,
            "map_kpa": map_values,
            "tps": np.linspace(10, 90, n_samples),
            "spark_f": 25 - np.abs(rpm_values - 4000) * 0.002,
            "spark_r": 24 - np.abs(rpm_values - 3800) * 0.002,
            "afr_error_f": np.random.uniform(-0.5, 0.5, n_samples),
            "afr_error_r": np.random.uniform(-0.5, 0.5, n_samples),
            "mode": "cruise",  # All cruise for simplicity
        })
        return df
    ...
    def test_surface_values_shape_matches_bins(self, sample_df):
        ...
        surface = build_surface(sample_df, spec, rpm_bins=rpm_bins, map_bins=map_bins)
...
class TestMinSamplesPerCell:
    ...
    def test_low_hit_cells_masked_to_none(self, sparse_df):
        ...
        surface = build_surface(sparse_df, spec, rpm_bins=rpm_bins, map_bins=map_bins)
        
        # Count non-None cells
        non_none_count = sum(
            1 for row in surface.values for val in row if val is not None
        )
