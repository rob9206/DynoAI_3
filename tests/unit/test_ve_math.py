...
class TestV2RatioModel:
    ...
    def test_on_target_no_correction():
        ...
        for target in test_targets:
            ...
            assert correction == 1.0, (
                f"Expected 1.0 for AFR {target}/{target}, got {correction}"
            )
    ...
    def test_lean_increases_fuel():
        ...
        for measured, target in lean_cases:
            ...
            assert correction > 1.0, (
                f"Lean ({measured}/{target}) should give correction > 1.0, got {correction}"
            )
    ...
    def test_rich_decreases_fuel():
        ...
        for measured, target in rich_cases:
            ...
            assert correction < 1.0, (
                f"Rich ({measured}/{target}) should give correction < 1.0, got {correction}"
            )
...
class TestV1LegacyModel:
    ...
    def test_7_percent_per_afr_point():
        ...
        # +1 AFR point (lean) -> +7%
        correction = calculate_ve_correction(14.0, target, version=MathVersion.V1_0_0)
        assert abs(correction - 1.07) < 0.0001

        # +2 AFR points (very lean) -> +14%
        correction = calculate_ve_correction(15.0, target, version=MathVersion.V1_0_0)
        assert abs(correction - 1.14) < 0.0001

        # -1 AFR point (rich) -> -7%
        correction = calculate_ve_correction(12.0, target, version=MathVersion.V1_0_0)
        assert abs(correction - 0.93) < 0.0001

        # -2 AFR points (very rich) -> -14%
        correction = calculate_ve_correction(11.0, target, version=MathVersion.V1_0_0)
    ...
    def test_linear_relationship():
        ...
        diffs = [
            corrections[i + 1] - corrections[i] for i in range(len(corrections) - 1)
        ]
        ...
        for target in targets:
            for delta in small_deltas:
                ...
                v1 = calculate_ve_correction(
                    measured, target, MathVersion.V1_0_0, clamp=False
                )
                v2 = calculate_ve_correction(
                    measured, target, MathVersion.V2_0_0, clamp=False
                )
...
class TestEdgeCases:
    ...
    def test_equal_at_all_afr_levels():
        ...
        for afr in [9.5, 10.0, 11.0, 12.0, 13.0, 14.0, 15.0, 16.0, 17.0, 18.0, 19.0]:
            ...
            assert correction == 1.0, (
                f"AFR {afr}/{afr} should give 1.0, got {correction}"
            )
