...
class TestGoldenValues:
    ...
    def test_ratio_formula_exact(golden_test_cases):
        ...
        for case in golden_test_cases:
            ...
            # Must be bit-exact
            assert result == expected, f"Ratio formula not exact for {case['scenario']}"
...
class TestVersionConsistency:
    ...
    def test_versions_differ_at_large_errors():
        ...
        assert diff_pct > 1.0, (
            f"Versions should differ by >1% at 3 AFR points, got {diff_pct:.2f}%"
        )
    ...
    def test_versions_similar_at_small_errors():
        ...
        diff_pct = abs(v2 - v1) / v2 * 100
        assert diff_pct < 0.5, (
            f"Versions should be within 0.5% at small errors, got {diff_pct:.2f}%"
        )
...
class TestRealWorldScenarios:
    ...
    def test_typical_cruise_tuning():
        ...
        assert -9.0 < pct < -7.0, (
            f"Cruise rich correction should be ~-8%, got {pct:.1f}%"
        )
...
class TestIntegrationWithRealData:
    ...
    def test_grid_analysis(sample_dyno_data):
        ...
        assert len(results) >= 4, (
            f"Expected at least 4 bins with data, got {len(results)}"
        )
