...
class TestTransientFuelAnalyzer:
    ...
    def test_afr_error_to_enrichment():
        ...
        analyzer_conservative = TransientFuelAnalyzer(
            target_afr=13.0, compensation_factor=0.25
        )
...
class TestIATCompensation:
    ...
    def test_cold_iat_increases_enrichment(
        ...
        cold_events = [
            e for e in result_cold.detected_events if e.iat_category in ["cold", "cool"]
        ]
    ...
    def test_recommendations_mention_iat(sample_cold_iat_data):
        ...
        iat_recommendations = [
            r for r in result.recommendations if "IAT" in r or "cold" in r.lower()
        ]
