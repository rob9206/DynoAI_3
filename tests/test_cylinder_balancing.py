...
from dynoai.constants import RPM_BINS, KPA_BINS
...
def create_imbalanced_records(num_records: int = 100) -> List[Dict[str, Optional[float]]]:
...
class TestAFRAggregation:
    ...
    def test_aggregate_front_cylinder(self):
        ...
        front_data = aggregate_cylinder_afr(records, "afr_meas_f", "afr_cmd_f", min_samples=3)
    ...
    def test_aggregate_rear_cylinder(self):
        ...
        rear_data = aggregate_cylinder_afr(records, "afr_meas_r", "afr_cmd_r", min_samples=3)
    ...
    def test_insufficient_samples_filtered(self):
        ...
        assert zero_cells > len(RPM_BINS) * len(KPA_BINS) * 0.8  # At least 80% should be zero
    ...
    def test_afr_cmd_grid_calculated(self):
        ...
        non_zero_cmd = sum(1 for row in front_data.afr_cmd_grid for val in row if val > 0.0)
...
class TestImbalanceAnalysis:
    ...
    def test_analyze_balanced_cylinders(self):
        ...
        assert analysis.cells_imbalanced < analysis.cells_analyzed * 0.2  # Less than 20%
    ...
    def test_imbalance_cell_severity(self):
        ...
        cell_high = ImbalanceCell(
            rpm_idx=0, kpa_idx=0, rpm=2000, kpa=50,
            front_afr=13.0, rear_afr=14.2, delta=1.2,
            front_samples=10, rear_samples=10
        )
        ...
        cell_medium = ImbalanceCell(
            rpm_idx=0, kpa_idx=0, rpm=2000, kpa=50,
            front_afr=13.0, rear_afr=13.8, delta=0.8,
            front_samples=10, rear_samples=10
        )
        ...
        cell_low = ImbalanceCell(
            rpm_idx=0, kpa_idx=0, rpm=2000, kpa=50,
            front_afr=13.0, rear_afr=13.5, delta=0.5,
            front_samples=10, rear_samples=10
        )
...
class TestCorrectionCalculation:
    ...
    def test_clamp_correction_exceeds_max(self):
        ...
        assert _clamp_correction(-0.06, max_pct) == pytest.approx(-0.03)  # Clamped to -3%
...
class TestOutputGeneration:
    ...
    def test_generate_balance_report(self):
        ...
        report = generate_balance_report(
            analysis, front_factors, rear_factors,
            BalanceMode.EQUALIZE, "test_input.csv"
        )
...
class TestProcessCylinderBalancing:
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("pathlib.Path.mkdir")
    def test_process_balanced_cylinders(self, mock_mkdir, mock_file_open, mock_write_csv):
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("pathlib.Path.mkdir")
    def test_process_imbalanced_cylinders(self, mock_mkdir, mock_file_open, mock_write_csv):
        ...
        assert result["front_corrections_applied"] + result["rear_corrections_applied"] > 0
    
    @patch("cylinder_balancing.write_correction_csv")
    @patch("builtins.open", new_callable=mock_open)
    @patch("pathlib.Path.mkdir")
    def test_process_invalid_mode_fallback(self, mock_mkdir, mock_file_open, mock_write_csv):
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("pathlib.Path.mkdir")
    def test_process_creates_output_files(self, mock_mkdir, mock_file_open, mock_write_csv):
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("pathlib.Path.mkdir")
    def test_process_respects_max_correction(self, mock_mkdir, mock_file_open, mock_write_csv):
