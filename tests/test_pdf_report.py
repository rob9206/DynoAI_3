...
class TestPDFReportGenerator(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.confidence_report = {
            ...
            'component_scores': {
                'coverage': {'score': 90.0, 'weight': 0.35, 'details': {'front_cells': 40, 'rear_cells': 40}},
                'consistency': {'score': 82.0, 'weight': 0.30, 'details': {'avg_mad': 0.45}},
                'quality': {'score': 84.0, 'weight': 0.35, 'details': {'anomalies': 1}},
            },
            'recommendations': ['Collect more data in cruise region', 'Monitor for knock'],
            'weak_areas': [],
        }
        
        # Create sample VE delta grid (9x5)
        self.ve_delta = [[1.5 if (i + j) % 2 == 0 else -1.2 for j in range(len(KPA_BINS))] for i in range(len(RPM_BINS))]
        
        # Create sample torque/hp maps
        self.torque_map = [[50.0 + i*5 + j*2 for j in range(len(KPA_BINS))] for i in range(len(RPM_BINS))]
        self.hp_map = [[40.0 + i*4 + j*1.5 for j in range(len(KPA_BINS))] for i in range(len(RPM_BINS))]
