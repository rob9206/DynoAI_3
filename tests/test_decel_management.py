...
class TestAFRAnalysis:
    ...
    def test_afr_analysis_rich_mixture(self):
        ...
        if events:  # May or may not detect depending on scenario
            ...
            for event in events:
                assert event.pop_likelihood < 0.1, "Rich mixture should have low pop likelihood"
    ...
    def test_afr_analysis_missing_data(self):
        ...
        for event in events:
            ...
            assert event.pop_likelihood == 0.0, "Pop likelihood should be 0 with missing data"
...
class TestEnrichmentCalculation:
    ...
    def test_severity_low_scaling(self):
        ...
        assert len(different_zones) > 0, "Some zones should show LOW < MEDIUM difference"
    ...
    def test_severity_high_scaling(self):
        ...
        for zone in medium:
            assert high[zone] > medium[zone], f"HIGH should be greater than MEDIUM for {zone}"
    ...
    def test_enrichment_clamping_max(self):
        ...
        for zone, value in enrichment.items():
            assert value <= MAX_ENRICHMENT_PCT, f"Enrichment should not exceed {MAX_ENRICHMENT_PCT}"
    ...
    def test_enrichment_minimum_floor(self):
        ...
        for zone, value in enrichment.items():
            assert value >= MIN_ENRICHMENT_PCT, f"Enrichment should be at least {MIN_ENRICHMENT_PCT}"
        ...
        for i, rpm in enumerate(RPM_BINS):
            for j, kpa in enumerate(KPA_BINS):
                if kpa > DECEL_KPA_MAX:
                    assert overlay[i][j] == 0.0, f"High kPa cell ({rpm}, {kpa}) should have 0 enrichment"
