...
def test_mts_packet_parsing():
    ...
    assert 22.0 <= sample_a.afr <= 22.8, f"Channel A AFR {sample_a.afr:.2f} out of range"
    assert 22.0 <= sample_b.afr <= 22.8, f"Channel B AFR {sample_b.afr:.2f} out of range"
...
def test_different_afr_values():
    ...
    for raw_val, expected_afr in test_cases:
        ...
        if sample:
            ...
            print(f"  Raw 0x{raw_val:04x} -> AFR {sample.afr:.2f} (expected {expected_afr:.1f}) {status}")
        else:
