...
from dynoai.constants import KPA_BINS, RPM_BINS, KPA_INDEX, RPM_INDEX, get_rpm_index, get_kpa_index
...
def process_knock_data(
    ...
    for r in records:
        ...
        is_knock = False
        if knock_val > 0: is_knock = True
        if retard_val > 0: is_knock = True
        ...
        if key not in analysis.cells:
            analysis.cells[key] = CellKnockStats(
                rpm_bin=RPM_BINS[r_idx], 
                kpa_bin=KPA_BINS[k_idx]
            )
...
def generate_timing_corrections(
    ...
    for (r_idx, k_idx), stats in analysis.cells.items():
        ...
        elif (stats.sample_count >= MIN_HITS_FOR_ADVANCE and 
              stats.knock_events == 0 and 
