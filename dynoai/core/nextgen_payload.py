...
class NextGenAnalysisPayload:
    ...
    ecu_model_notes: List[str] = field(default_factory=lambda: list(ECU_MODEL_NOTES))
    ...
    def has_per_cylinder_data(self) -> bool:
        ...
        return (
            "spark_front" in self.surfaces or 
            "spark_rear" in self.surfaces or
            "afr_error_front" in self.surfaces
        )
...
def build_nextgen_payload(
    ...
    spark_valley_findings: List[Any],  # List[SparkValleyFinding] from spark_valley
    ...
    if hasattr(normalization_result, 'warning_codes'):
        ...
        warning_codes = [w.code for w in normalization_result.structured_warnings]
