...
class ModeLabeledFrame:
    ...
    config_used: ModeDetectionConfig = field(default_factory=ModeDetectionConfig)
    ...
    def mode_distribution(self) -> Dict[str, float]:
        ...
        if total == 0:
            return {}
        return {mode: count / total * 100 for mode, count in self.summary_counts.items()}
    ...
    def to_summary_dict(self) -> Dict:
        ...
        return {
            ...
            "distribution_pct": {k: round(v, 1) for k, v in self.mode_distribution.items()},
            "durations_sec": {k: round(v, 2) for k, v in self.mode_durations_sec.items()},
            "total_samples": self.total_samples,
            "total_duration_sec": round(sum(self.mode_durations_sec.values()), 2),
        }
...
def compute_derivatives(
    ...
    if "tps" in df.columns:
        ...
        df["tps_dot"] = (
            tps_dot_raw.rolling(window=config.derivative_window, min_periods=1)
            .mean()
            .fillna(0)
        )
    else:
    ...
    if "map_kpa" in df.columns:
        ...
        # Apply smoothing window
        df["map_dot"] = (
            map_dot_raw.rolling(window=config.derivative_window, min_periods=1)
            .mean()
            .fillna(0)
        )
    else:
...
def classify_sample(
    ...
    if (
        iat > config.iat_soak_threshold and
        rpm < config.rpm_soak_ceiling and
        tps < config.tps_soak_ceiling
    ):
        return ModeTag.HEAT_SOAK
    
    # 2. Check for tip-in transient
    if (
        tps_dot > config.tpsdot_tipin_threshold or
        map_dot > config.mapdot_tipin_threshold
    ):
        return ModeTag.TIP_IN
    
    # 3. Check for tip-out transient
    if (
        tps_dot < config.tpsdot_tipout_threshold or
        map_dot < config.mapdot_tipout_threshold
    ):
    ...
    # 6. Check for idle
    if (
        rpm < config.rpm_idle_ceiling and
        tps < config.tps_idle_ceiling and
        map_kpa < config.map_idle_ceiling
    ):
        return ModeTag.IDLE
...
def get_steady_state_mask(labeled_df: pd.DataFrame) -> pd.Series:
    ...
    Returns:
        Boolean Series mask
    """
    excluded_modes = [ModeTag.TIP_IN.value, ModeTag.TIP_OUT.value, ModeTag.HEAT_SOAK.value]
