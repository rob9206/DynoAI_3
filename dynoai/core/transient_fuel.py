...
class TransientFuelAnalyzer:
    ...
    def __init__(
        ...
        self.compensation_factor = max(
            0.1, min(compensation_factor, 1.0)
        )  # Clamp 0.1-1.0
        ...
        self._env_corrector = (
            EnvironmentalCorrector() if environmental_conditions else None
        )
        ...
        if environmental_conditions:
            self._env_correction = self._env_corrector.calculate(
    ...
    def _calculate_rates(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["map_rate"] = (
            df["map_rate"].rolling(window=window, center=True, min_periods=1).mean()
        )

        df["tps_rate"] = df["tps"].diff().fillna(0) / dt
        df["tps_rate"] = (
            df["tps_rate"].rolling(window=window, center=True, min_periods=1).mean()
        )
    ...
    def detect_transient_events(self, df: pd.DataFrame) -> List[TransientEvent]:
        ...
        accel_mask = (df["map_rate"].abs() > self.map_rate_threshold) | (
        ...
        for event in events:
            ...
            if 0 <= bin_idx < len(enrichment):
                ...
                needed_enrichment = self._afr_error_to_enrichment(
                    event.afr_error_avg, iat_c=event.avg_iat_c
                )
    ...
    def calculate_wall_wetting_compensation(
        ...
        for range_name, rpm_min, rpm_max in rpm_ranges:
            ...
            iat_adjusted_correction = (
                base_correction * iat_density_factor * iat_ww_factor
            )
    ...
    def calculate_tau_wall_wetting(
        ...
        for range_name, rpm_min, rpm_max in rpm_ranges:
            ...
            x_estimate = max(
                0.05, min(x_estimate, 0.50)
            )  # Wider range for cold conditions
            ...
            tau_estimate = (
                base_tau * rate_factor * iat_ww_factor
            )  # IAT increases tau when cold
            tau_estimate = max(
                0.2, min(tau_estimate, 8.0)
            )  # Wider range for cold conditions
    ...
    def calculate_tau_enrichment(
        ...
        enrichment = (
            tau_params.x_fraction * tau_params.tau_seconds * (map_rate / 100.0) * 100.0
        )
    ...
    def _calculate_3d_enrichment(
        ...
        for rpm_idx in range(len(rpm_bins) - 1):
            for map_idx in range(len(map_rate_bins) - 1):
                ...
                map_rate_center = (
                    map_rate_bins[map_idx] + map_rate_bins[map_idx + 1]
                ) / 2
                ...
                cell_events = [
                    ...
                    for e in events
                    if (
                        rpm_bins[rpm_idx] <= e.avg_rpm < rpm_bins[rpm_idx + 1]
                        and map_rate_bins[map_idx]
                        <= e.peak_map_rate
                        < map_rate_bins[map_idx + 1]
                        and e.event_type == "accel"
                    )
                ]
    ...
    def _afr_error_to_enrichment(
        self, afr_error: float, iat_c: Optional[float] = None
    ) -> float:
        """
    ...
    def _generate_recommendations(
        ...
        afr_errors: List[Tuple[float, float]],
    ) -> List[str]:
    ...
    def export_power_vision(
        ...
        with open(output_path, "w") as f:
            ...
            if self._env_correction is not None:
                ...
                f.write(
                    f"# Environmental Correction: {(env.total_correction - 1) * 100:+.1f}%\n"
                )
            ...
            f.write(
                f"Wall Wetting Coefficient: {self.iat_wall_wetting_coeff} (per °C)\n"
            )
            f.write(
                "# Cold air (<25°C) increases enrichment due to higher density and wall wetting\n"
            )
            ...
            f.write(
                "# Both X and tau are scaled by IAT: cold increases values, hot decreases\n"
            )
            f.write(
                "# Enrichment formula: enrich% = X * tau * (MAP_rate/100) * 100\n\n"
            )
...
if __name__ == "__main__":
    ...
    rpm[(time >= 5) & (time <= 10)] = (
        3000 + (time[(time >= 5) & (time <= 10)] - 5) * 800
    )

    map_kpa = np.ones_like(time) * 50
    map_kpa[(time >= 5) & (time <= 10)] = (
        50 + (time[(time >= 5) & (time <= 10)] - 5) * 30
    )
    ...
    afr[(time >= 5) & (time <= 10)] = (
        13.0 + (time[(time >= 5) & (time <= 10)] - 5) * 0.6
    )
    ...
    for i, event in enumerate(result.detected_events, 1):
        print(
            f"  Event {i}: {event.event_type} ({event.severity}) at t={event.start_time:.1f}s"
        )
        print(f"    Peak MAP rate: {event.peak_map_rate:.1f} kPa/s")
        print(f"    Peak TPS rate: {event.peak_tps_rate:.1f} %/s")
        print(
            f"    AFR error: {event.afr_error_avg:.2f} (peak: {event.afr_error_peak:.2f})"
        )
