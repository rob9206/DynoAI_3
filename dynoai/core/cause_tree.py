...
def _get_high_map_slice_stats(surface: Surface2D) -> Dict[str, Any]:
    ...
    high_map_indices = [i for i, m in enumerate(map_bins) if m >= HIGH_MAP_THRESHOLD]
...
def _get_midrange_rpm_stats(surface: Surface2D, rpm_low: int = MID_RPM_LOW, rpm_high: int = MID_RPM_HIGH) -> Dict[str, Any]:
    """
    ...
    # Find indices
    high_map_indices = [i for i, m in enumerate(map_bins) if m >= HIGH_MAP_THRESHOLD]
    mid_rpm_indices = [i for i, r in enumerate(rpm_bins) if rpm_low <= r <= rpm_high]
...
def _check_knock_correlation_with_valley(
    ...
    if knock_surface is None:
        result["evidence"].append("No knock data available - cannot confirm knock correlation")
        return result
    ...
    valley_rpm_indices = [i for i, r in enumerate(rpm_bins) if valley_rpm_low <= r <= valley_rpm_high]
    high_map_indices = [i for i, m in enumerate(map_bins) if m >= HIGH_MAP_THRESHOLD]
    ...
    if knock_in_band:
        ...
        avg_outside = sum(knock_outside_band) / len(knock_outside_band) if knock_outside_band else 0
        ...
        if result["knock_peak_rpm"] and valley_rpm_low <= result["knock_peak_rpm"] <= valley_rpm_high:
            result["has_correlation"] = True
            result["evidence"].append(
...
def _check_transient_issues(
    ...
    if is_lean or high_variance:
        ...
        # Build evidence
        evidence.append(f"TIP_IN mode: {tip_in_count} samples ({transient_pct:.1f}% of data)")
        
        if is_lean:
            evidence.append(f"Mean AFR error is lean: +{mean_error:.2f} (threshold: +{TRANSIENT_AFR_LEAN_THRESHOLD})")
            evidence.append("Lean tip-in indicates insufficient transient enrichment or wall wetting compensation")
            confidence += 0.15
        
        if high_variance:
            evidence.append(f"High AFR variance: {variance:.2f} (5th-95th percentile spread)")
            evidence.append("High variance suggests inconsistent fuel delivery during transients")
            confidence += 0.1
        
        # ECU coupling insight
        evidence.append("ECU context: VE is a correction layer; transient enrichment compensates for wall wetting lag")
        ...
        if has_knock and "knock_activity" in surfaces:
            ...
            if knock_surface.stats.max and knock_surface.stats.max > 0:
                evidence.append("Knock activity detected - lean tip-in may cause knock spikes")
                evidence.append("ECU coupling: Lean combustion reduces knock margin")
                confidence += 0.1
        elif not has_knock:
            evidence.append("No knock data available - cannot assess knock correlation with lean tip-in")
            confidence -= 0.05
...
def _check_knock_issues(
    ...
    if spark_valley:
        for finding in spark_valley:
            ...
            evidence.append(f"Spark valley at {finding.rpm_center} RPM in high-MAP band")
            evidence.append(f"Valley depth: {finding.depth_deg:.1f}° (threshold: {KNOCK_VALLEY_MIN_DEPTH}°)")
            evidence.append(f"Valley RPM range: {finding.rpm_band[0]}-{finding.rpm_band[1]} RPM")
            evidence.append(f"Timing drops from {finding.pre_valley_deg:.1f}° to {finding.valley_min_deg:.1f}° then rises to {finding.post_valley_deg:.1f}°")
            
            # ECU coupling insight
            evidence.append("ECU context: Spark is base + modifiers; knock always has authority to retard timing")
            
            # Significant depth increases confidence
            if finding.depth_deg >= KNOCK_VALLEY_SIGNIFICANT_DEPTH:
                confidence += 0.15
                evidence.append(f"Valley depth is significant (>= {KNOCK_VALLEY_SIGNIFICANT_DEPTH}°)")
            
            # Check knock correlation
            if has_knock and knock_surface is not None:
                knock_corr = _check_knock_correlation_with_valley(knock_surface, finding)
                
                if knock_corr["has_correlation"]:
                    confidence += 0.2
                    evidence.append("Knock activity correlates with spark valley location")
                    evidence.extend(knock_corr["evidence"])
                    evidence.append("ECU coupling: Knock-limited timing confirmed by knock sensor data")
                else:
                    evidence.append("Knock data present but no strong correlation with valley")
                    evidence.append("Valley may be calibrated conservatively or knock sensor sensitivity low")
            elif not has_knock:
                confidence -= 0.1
                evidence.append("No knock data available - cannot confirm knock correlation")
                evidence.append("Lower confidence: valley could be knock-limited or conservatively calibrated")
        ...
        if front and rear:
            ...
            if depth_diff >= KNOCK_CYLINDER_ASYMMETRY_THRESHOLD:
                ...
                if has_knock and knock_surface is not None:
                    ...
                    evidence.append("Check per-cylinder knock data to confirm rear-dominant knock")
                else:
                    confidence -= 0.1
                    evidence.append("No knock data to confirm cylinder asymmetry")
...
def _check_fuel_model_issues(
    ...
    for surface_id in ["afr_error_front", "afr_error_rear", "afr_error_global"]:
...
def _check_temp_issues(
    ...
    if heat_soak_count >= TEMP_HEAT_SOAK_MIN_SAMPLES or heat_soak_pct > 5:
        ...
        evidence.append(f"Heat soak detected: {heat_soak_count} samples ({heat_soak_pct:.1f}% of data)")
        evidence.append("High IAT + low airflow conditions trigger temperature trims")
        evidence.append("ECU context: Temperature trims reduce spark timing for knock safety")
        
        # Analyze spark reduction at high MAP
        if spark_surface is not None:
            high_map_stats = _get_high_map_slice_stats(spark_surface)
            
            if high_map_stats["mean"] is not None and high_map_stats["min"] is not None:
                spark_range = (high_map_stats["max"] or 0) - (high_map_stats["min"] or 0)
                
                if spark_range > TEMP_SPARK_REDUCTION_THRESHOLD:
                    evidence.append(f"Spark timing varies {spark_range:.1f}° across high-MAP band")
                    evidence.append(f"Minimum spark: {high_map_stats['min']:.1f}° at {high_map_stats['rpm_at_min']} RPM")
                    evidence.append("Broad spark reduction suggests temperature trims active")
                    confidence += 0.15
        
        # IAT availability affects confidence
        if has_iat:
            evidence.append("IAT data available - can correlate spark reduction with temperature")
            confidence += 0.1
        else:
            evidence.append("No IAT data - cannot directly correlate spark with temperature")
            evidence.append("Heat soak mode detected via other signals (low RPM, low airflow)")
            confidence -= 0.1
    ...
    if spark_surface is not None:
        ...
        if (high_map_stats["mean"] is not None and 
            midrange_stats["mean"] is not None and
            high_map_stats["valid_cells"] >= 5):
            ...
            if spark_max - spark_min < 3.0 and spark_mean < 25:  # Relatively flat and low
                evidence = [
                ...
                if not has_iat:
                    evidence.append("Add IAT logging to confirm temperature correlation")
...
def build_cause_tree(
    ...
    analysis_notes.append("ECU Model: VE is correction layer, spark has knock authority, VE/spark coupled via combustion")
    
    # Run ECU-coupling-aware hypothesis generators
    # Rule A: Transient fuel issues
    all_hypotheses.extend(_check_transient_issues(mode_summary, surfaces, has_knock))
    
    # Rule B: Knock-limited timing
    all_hypotheses.extend(_check_knock_issues(surfaces, spark_valley, has_knock))
    ...
    # Add analysis notes
    total_samples = sum(mode_summary.values())
    analysis_notes.append(f"Analyzed {total_samples} samples across {len(surfaces)} surfaces")
    ...
    if spark_valley:
        analysis_notes.append(f"Detected {len(spark_valley)} spark valley finding(s)")
    
    # Note graceful degradation
    if not has_knock:
        analysis_notes.append("Graceful degradation: Knock-related confidence reduced without knock data")
