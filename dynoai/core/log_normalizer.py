...
WARNING_CODES = {
    ...
    "missing_channel:rpm": "Required channel 'rpm' not found",
    "missing_channel:map_kpa": "Required channel 'map_kpa' not found",
    "missing_channel:tps": "Throttle position (tps) not found - mode detection may be less accurate",
    "missing_channel:iat": "Intake air temp (iat) not found - heat soak detection disabled",
    "missing_channel:ect": "Coolant temp (ect) not found - thermal analysis limited",
    "missing_channel:afr_meas": "No AFR measurement found - AFR analysis disabled",
    "missing_channel:afr_meas_f": "Front cylinder AFR not found - using global AFR",
    "missing_channel:afr_meas_r": "Rear cylinder AFR not found - using global AFR",
    "missing_channel:afr_cmd": "No commanded AFR found - using stoich (14.7) default",
    "missing_channel:afr_cmd_f": "Front cylinder AFR target not found",
    "missing_channel:afr_cmd_r": "Rear cylinder AFR target not found",
    "missing_channel:spark": "No spark timing found - spark valley analysis disabled",
    "missing_channel:spark_f": "Front cylinder spark not found - using global spark",
    "missing_channel:spark_r": "Rear cylinder spark not found - using global spark",
    ...
    "degraded:per_cylinder_spark": "Using global spark for both cylinders (no per-cylinder data)",
    "degraded:afr_cmd_default": "No commanded AFR found, using stoich (14.7) as default",
}
...
COLUMN_ALIASES: Dict[str, List[str]] = {
    # Engine speed
    "rpm": ["rpm", "engine_rpm", "engine rpm", "enginespeed", "engine speed", "n"],
    
    # Manifold pressure
    "map_kpa": ["map_kpa", "map kpa", "map", "manifold_pressure", "manifold pressure", 
                "boost_kpa", "intake_pressure"],
    
    # Throttle position
    "tps": ["tps", "throttle", "throttle_position", "throttle position", "tp", 
            "throttle_%", "throttle%"],
    
    # Intake air temperature
    "iat": ["iat", "intake_air_temp", "intake air temp", "iat_f", "iat_c", 
            "air_temp", "inlet_temp"],
    
    # Engine coolant temperature
    "ect": ["ect", "coolant_temp", "coolant temp", "engine_temp", "engine temp",
            "et", "water_temp"],
    
    # AFR measured - front cylinder
    "afr_meas_f": ["afr_meas_f", "afr meas f", "wbo2_f", "wbo2 f", "afr_f", 
                   "afr_front", "afr front", "lambda_f", "o2_f"],
    
    # AFR measured - rear cylinder
    "afr_meas_r": ["afr_meas_r", "afr meas r", "wbo2_r", "wbo2 r", "afr_r",
                   "afr_rear", "afr rear", "lambda_r", "o2_r"],
    
    # AFR measured - global/single sensor
    "afr_meas": ["afr_meas", "afr meas", "afr", "wbo2", "lambda", "o2",
                 "air_fuel_ratio", "air/fuel ratio"],
    
    # AFR commanded - front cylinder
    "afr_cmd_f": ["afr_cmd_f", "afr cmd f", "afr_target_f", "afr target f",
                  "lambda_target_f", "cmd_afr_f"],
    
    # AFR commanded - rear cylinder
    "afr_cmd_r": ["afr_cmd_r", "afr cmd r", "afr_target_r", "afr target r",
                  "lambda_target_r", "cmd_afr_r"],
    ...
    "spark_f": ["spark_f", "spark f", "advance_f", "advance f", "timing_f",
                "spark_adv_f", "ignition_f", "ign_f"],
    
    # Spark timing - rear cylinder
    "spark_r": ["spark_r", "spark r", "advance_r", "advance r", "timing_r",
                "spark_adv_r", "ignition_r", "ign_r"],
    
    # Spark timing - global
    "spark": ["spark", "advance", "timing", "spark_adv", "ignition", "ign",
              "spark_advance", "ignition_timing"],
    
    # Knock sensor
    "knock": ["knock", "knock_sensor", "knock sensor", "knock_count", 
              "knock_retard", "kr"],
...
def normalize_dataframe(
    ...
    # AFR presence
    presence.has_per_cylinder_afr = (
        "afr_meas_f" in df.columns and "afr_meas_r" in df.columns
    )
    ...
    if not presence.has_per_cylinder_afr and presence.has_global_afr:
        ...
        warnings.append("Using global AFR for both cylinders (no per-cylinder data)")
    
    # Spark presence
    presence.has_per_cylinder_spark = (
        "spark_f" in df.columns and "spark_r" in df.columns
    )
    ...
    if not presence.has_per_cylinder_spark and presence.has_global_spark:
        ...
        warnings.append("Using global spark for both cylinders (no per-cylinder data)")
    ...
    if "afr_cmd_f" not in df.columns:
        ...
        df["afr_cmd_r"] = 14.7
        columns_derived.extend(["afr_cmd_f", "afr_cmd_r"])
        warnings.append("No commanded AFR found, using stoich (14.7) as default")
    
    # Knock presence
    presence.has_per_cylinder_knock = (
        "knock_f" in df.columns and "knock_r" in df.columns
    )
    presence.has_knock = (
        presence.has_per_cylinder_knock or "knock" in df.columns
    )
    ...
    for col in OPTIONAL_COLUMNS:
        if col not in df.columns and col not in columns_derived:
            if col not in ["time_ms", "timestamp"]:  # Don't report both time variants
                columns_missing.append(col)
    ...
    if "rpm" not in columns_found:
        structured_warnings.append(ChannelWarning(
    ...
    if not presence.has_per_cylinder_spark and not presence.has_global_spark:
        ...
        warnings.append("No spark data found - spark valley analysis will be disabled")
    ...
    if not presence.has_knock:
        ...
        warnings.append("No knock data found - knock-based reasoning will be disabled")
...
def get_channel_readiness(norm_result: NormalizationResult) -> ChannelReadiness:
    """
    ...
    recommended_channels = [
        ...
        },
        {
            "name": "afr_meas",
            "label": "AFR Measured",
            "present": presence.can_analyze_afr,
            "required": False,
            "impact": "AFR error surfaces",
            "note": "per-cylinder" if presence.has_per_cylinder_afr else "global" if presence.has_global_afr else "missing",
        },
        {
            "name": "afr_cmd",
            "label": "AFR Commanded",
            "present": "afr_cmd" in norm_result.columns_found or "afr_cmd_f" in norm_result.columns_found,
            "required": False,
            "impact": "AFR error accuracy",
        },
        {
            "name": "spark",
            "label": "Spark Timing",
            "present": presence.can_analyze_spark,
            "required": False,
            "impact": "Spark valley detection",
            "note": "per-cylinder" if presence.has_per_cylinder_spark else "global" if presence.has_global_spark else "missing",
        },
        {
            "name": "knock",
            "label": "Knock Sensor",
    ...
    if confidence >= 0.9:
        trust_summary = "Excellent data quality - all analyses available with high confidence"
    elif confidence >= 0.7:
