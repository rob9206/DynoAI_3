...
class WeightedBinAccumulator:
    ...
    def __init__(
        ...
        self._cells: List[List[CellAccumulator]] = [
            [CellAccumulator() for _ in range(len(y_axis))]
    ...
    def statistics(self) -> dict:
        ...
        cells_with_data = sum(
            1 for x in range(len(self.x_axis))
            for y in range(len(self.y_axis))
        ...
        return {
            ...
            "rejection_rate": (
                (self._total_samples - self._accepted_samples) / self._total_samples * 100
                if self._total_samples > 0 else 0.0
            ),
            "total_cells": total_cells,
            "cells_with_data": cells_with_data,
            "coverage_pct": cells_with_data / total_cells * 100 if total_cells > 0 else 0.0,
