...
class LowpassFilter(SignalFilter):
    ...
    def filter(self, samples: List[FilteredSample]) -> List[FilteredSample]:
        ...
        for i in range(1, len(samples)):
            ...
            samples[i].value = (
                alpha * samples[i].value + (1.0 - alpha) * prev_sample.value
            )
...
class TimeAwareMinMaxFilter(SignalFilter):
    ...
    def filter(self, samples: List[FilteredSample]) -> List[FilteredSample]:
        ...
        for i, sample in enumerate(samples):
            ...
            is_out_of_bounds = (
                sample.value < self.min_val or sample.value > self.max_val
            )
...
def samples_from_arrays(
    ...
    return [FilteredSample(time_ms=t, value=v) for t, v in zip(times_ms, values)]
