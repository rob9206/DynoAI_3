...
def _smooth_curve(
    ...
    for i in range(n):
        ...
        window_vals = [filled[j] for j in range(start, end) if filled[j] is not None]
...
def detect_spark_valley(
    ...
    if not high_map_indices:
        ...
        high_map_indices = list(range(max(0, len(map_bins) - 2), len(map_bins)))
        evidence.append(f"Using top {len(high_map_indices)} MAP bins (no bins >= {high_map_min_kpa} kPa)")
    
    if not high_map_indices:
        return []
    
    # Calculate average MAP value used
    avg_map = sum(map_bins[i] for i in high_map_indices) / len(high_map_indices)
    ...
    evidence.append(f"Analyzed {len(high_map_indices)} MAP bins (avg {avg_map:.0f} kPa)")
    ...
    if valid_count < 5:
        evidence.append(f"Insufficient data: only {valid_count} valid RPM points")
        return []
    
    evidence.append(f"Data coverage: {valid_count}/{total_count} RPM bins ({coverage_pct:.0f}%)")
    ...
    if valley_result is None:
        evidence.append(f"No valley detected (min depth threshold: {min_depth_deg}°)")
        return []
    ...
    evidence.append(f"Pre-valley timing: {pre_peak:.1f}° | Post-valley: {post_peak:.1f}°")
...
def _calculate_valley_confidence(
    ...
    if hit_counts:
        valley_hits = hit_counts[valley_idx] if valley_idx < len(hit_counts) else 0
...
def detect_valleys_multi_cylinder(
    ...
    if "spark_front" in surfaces:
        findings.extend(detect_spark_valley(
...
def compare_cylinder_valleys(
    ...
    if abs(depth_diff) >= 2.0:
        ...
        observations.append(f"{deeper_cyl} cylinder valley is {abs(depth_diff):.1f}° deeper")
    
    if abs(rpm_diff) >= 500:
        earlier_cyl = "front" if rpm_diff > 0 else "rear"
        observations.append(f"{earlier_cyl} cylinder valley occurs at lower RPM")
    
    if abs(min_timing_diff) >= 3.0:
        more_retarded = "rear" if min_timing_diff < 0 else "front"
        observations.append(f"{more_retarded} runs {abs(min_timing_diff):.1f}° more retarded at valley")
