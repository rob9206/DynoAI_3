...
class TestStep:
    ...
    test_type: str = "general"  # steady_state_sweep, wot_pull, transient_rolloff, idle_hold
    constraints: str = ""
...
class NextTestPlan:
    ...
    def get_street_steps(self) -> List[TestStep]:
        ...
        return [s for s in self.steps if s.test_type in ["transient_rolloff", "steady_state_sweep"]]
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "coverage_gaps_detailed": [g.to_dict() for g in self.coverage_gaps_detailed] if self.coverage_gaps_detailed else [],
            "total_estimated_pulls": self.total_estimated_pulls,
            "dyno_step_count": len(self.get_dyno_steps()),
            "street_step_count": len(self.get_street_steps()),
        }
...
class CoverageGap:
    ...
    description: str
    region_type: str = "general"  # "high_map_midrange", "idle_low_map", "tip_in", "general"
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "coverage_pct": round((self.total_cells - self.empty_cells) / self.total_cells * 100, 1) if self.total_cells > 0 else 0,
            "impact": self.impact,
...
def identify_coverage_gaps(
    ...
    ref_surface = None
    for key in ["spark_front", "spark_rear", "afr_error_front", "afr_error_rear", 
                "spark_global", "afr_error_global"]:
        if key in surfaces:
    ...
    high_impact_regions = [
        ...
        ("High MAP midrange (dyno-critical)", (2500, 4500), (80, 100), "high", "high_map_midrange"),
        ("WOT torque peak", (4000, 5500), (85, 100), "high", "high_map_midrange"),
        ("High MAP low-mid RPM", (2000, 3000), (80, 100), "high", "high_map_midrange"),
    ...
    for region_name, rpm_range, map_range, impact, region_type in high_impact_regions:
        ...
        rpm_indices = [i for i, r in enumerate(rpm_bins) if rpm_low <= r <= rpm_high]
        map_indices = [i for i, m in enumerate(map_bins) if map_low <= m <= map_high]
...
def generate_inertia_dyno_tests(
    ...
    if high_map_gaps:
        ...
        for rpm_low, rpm_high in sorted(rpm_ranges):
            ...
            if rpm_high <= 4000:
                ...
                gear_suggestion = "5th or 6th gear (higher gear = slower ramp for top end)"
            ...
            steps.append(TestStep(
                ...
                required_channels=["afr_meas_f", "afr_meas_r", "spark_f", "spark_r", "knock"],
    ...
    if mode_summary:
        ...
        if wot_samples < 100:
            steps.append(TestStep(
                ...
                required_channels=["afr_meas_f", "afr_meas_r", "spark_f", "spark_r", "knock", "iat"],
    ...
    steps.append(TestStep(
        ...
        required_channels=["afr_meas_f", "afr_meas_r", "spark_f", "spark_r"],
...
def generate_street_tests(
    ...
    if tipin_gaps:
        steps.append(TestStep(
            ...
            constraints=(
                "Route script: From steady 2500 RPM cruise, smoothly roll on throttle "
    ...
    if idle_gaps:
        steps.append(TestStep(
            ...
            constraints=(
                "Route script: Park in safe location. Log 60+ seconds at idle. "
    ...
    if mode_summary:
        ...
        if cruise_samples < 200:
            steps.append(TestStep(
                ...
                test_type="steady_state_sweep",
                constraints=(
                    "Route script: Find a flat road or highway. Hold steady speeds: "
    ...
    steps.append(TestStep(
        ...
        test_type="transient_rolloff",
        constraints=(
            "Route script: Accelerate to 4000+ RPM, then fully lift throttle. "
    ...
    if mode_summary:
        ...
        if heat_soak_samples < 50:
            steps.append(TestStep(
                ...
                test_type="steady_state_sweep",
                constraints=(
                    "Route script: After a spirited drive, park and idle for 2-3 minutes "
    ...
    for hyp in hypotheses:
        ...
        if hyp.category == HypothesisCategory.TRANSIENT:
            ...
            steps.append(TestStep(
                ...
                required_channels=["knock", "spark_f", "spark_r", "afr_meas_f", "afr_meas_r"],
            ...
            steps.append(TestStep(
                ...
                required_channels=["iat", "ect", "afr_meas_f", "afr_meas_r"],
...
def generate_test_plan(
    ...
    if cumulative_coverage is None:
        ...
        total_cells = sum(s.stats.total_cells for s in surfaces.values() if s.stats)
        covered_cells = sum(s.stats.non_nan_cells for s in surfaces.values() if s.stats)
        current_coverage = (covered_cells / total_cells * 100) if total_cells > 0 else 0.0
    else:
    ...
    if cause_tree and cause_tree.hypotheses:
        ...
        top_hyps = [h for h in cause_tree.hypotheses if h.confidence >= 0.7][:2]
        if top_hyps:
    ...
    if current_coverage < 70:
        rationale_parts.append("Low overall coverage - prioritizing high-impact regions")
    
    priority_rationale = "; ".join(rationale_parts) if rationale_parts else "Standard coverage improvement plan"
