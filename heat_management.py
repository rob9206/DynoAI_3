...
def detect_soak_events(
    ...
    for i, r in enumerate(records):
        ...
        is_soak_condition = (
            iat >= threshold_f and 
            rpm < SOAK_RPM_CEILING and 
            tps < SOAK_TPS_CEILING
        )
        ...
        if is_soak_condition and not in_event:
            ...
            if duration > 2.0: # Minimum 2 seconds to matter
                ...
                events.append(SoakEvent(
                    ...
                    rpm_range=(min(x.get('rpm',0) for x in segment), max(x.get('rpm',0) for x in segment)),
...
def generate_heat_correction_overlay(
    ...
    for event in events:
        ...
        if min_r_idx == -1: min_r_idx = 0
        if max_r_idx == -1: max_r_idx = len(rpm_bins) - 1
    ...
    for (r, k), errors in cell_errors.items():
        ...
        if avg_correction < -0.10: avg_correction = -0.10
        if avg_correction > 0.0: avg_correction = 0.0 # Should only be removing fuel
