...
def get_analysis(run_id: str):
    ...
    if cached is None:
        return (
            jsonify({
                "error":
                f"NextGen analysis not found for run {run_id}",
                "hint":
                f"Generate analysis first: POST /api/nextgen/{run_id}/generate",
            }),
            404,
        )
...
def download_analysis(run_id: str):
    ...
    if payload_path is None or not payload_path.exists():
        return (
            jsonify({
                "error":
                f"NextGen analysis file not found for run {run_id}",
                "hint":
                f"Generate analysis first: POST /api/nextgen/{run_id}/generate",
            }),
            404,
...
def get_hypotheses(run_id: str):
    ...
    # Apply filters
    filtered = [
        h for h in hypotheses if h.get("confidence", 0) >= min_confidence
    ]

    if category_filter:
        filtered = [
            h for h in filtered if h.get("category") == category_filter
        ]
