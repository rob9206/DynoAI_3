...
def start_tuning_session():
    ...
    try:
        ...
        config = TuningSessionConfig(
            ...
            max_correction_per_iteration_pct=data.get("max_correction_per_iteration_pct", 15.0),
            oscillation_detection_enabled=data.get("oscillation_detection_enabled", True),
    ...
    try:
        ...
        for session_id, session in orchestrator.sessions.items():
            sessions_list.append({
                ...
                "duration_sec": (session.end_time or 0) - session.start_time if session.end_time else None,
    ...
    try:
        ...
        if session.iterations:
            ...
            final_metrics = {
                ...
                "convergence_rate": "fast" if session.current_iteration <= 3 else "normal" if session.current_iteration <= 6 else "slow",
                "time_to_convergence_sec": round((session.end_time or 0) - session.start_time, 1) if session.end_time else None,
            }
        
        # VE evolution
        ve_evolution = {}
        if session.baseline_ve is not None and session.current_ve_front is not None:
            initial_error = ((session.iterations[0].ve_table_front - session.baseline_ve) / session.baseline_ve * 100).mean() if session.iterations else 0
            final_error = ((session.current_ve_front - session.baseline_ve) / session.baseline_ve * 100).mean()
            ve_evolution = {
