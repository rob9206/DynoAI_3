...
class EnvironmentalConditions:
    ...
    def is_standard_day(self) -> bool:
        ...
        return (
            70 <= self.ambient_temp_f <= 85 and
            29.5 <= self.barometric_pressure_inhg <= 30.3 and
...
class TuningSession:
    ...
    afr_targets: Dict[str, float] = field(default_factory=dict)  # e.g., {"cruise": 13.8, "wot": 12.8}
...
class CylinderImbalancePattern:
    ...
    primary_cause: str = "unknown"     # "firing_interval", "heat_differential", "exhaust_scavenging"
...
class DecelPoppingPattern:
    ...
    enrichment_zones: List[Tuple[int, int, int, int, float]] = field(default_factory=list)
...
class TrainingDataset:
    ...
    cylinder_imbalance_patterns: List[CylinderImbalancePattern] = field(default_factory=list)
