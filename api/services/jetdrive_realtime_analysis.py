...
class QualityMetrics:
    ...
    def get_freshness(self, current_time: float) -> dict[str, float]:
        ...
        return {ch: current_time - ts for ch, ts in self.channel_last_update.items()}
    ...
    def _detect_alerts(self, data: dict[str, Any], current_time: float) -> None:
        ...
        for channel, staleness in freshness.items():
            if (
                ...
                and staleness > CHANNEL_STALE_THRESHOLD_SEC
                and channel in REQUIRED_CHANNELS
            ):
                self._add_alert(
                    Alert(
