...
class QualityMetrics:
    ...
    channel_values: dict[str, list[float]] = field(
        default_factory=dict)  # Recent values for variance
    ...
    def compute_score(self, current_time: float, coverage_pct: float) -> float:
        ...
        if freshness:
            ...
            freshness_score = max(0,
                                  100 - avg_freshness * 20)  # 5 sec = 0 score
        else:
            freshness_score = 0

        # Missing channels penalty
        missing_penalty = (len(self.missing_channels) * 20
                           )  # -20 per missing required channel
        ...
        # Weighted average
        self.overall_score = (QUALITY_WEIGHT_FRESHNESS * freshness_score +
...
class RealtimeAnalysisEngine:
    ...
    def _bin_rpm_map(self, rpm: float,
                     map_kpa: float) -> tuple[int, int] | None:
        """
    ...
    def _update_coverage(self, rpm: float, map_kpa: float,
    ...
    def _update_quality(self, data: dict[str, Any],
                        current_time: float) -> None:
    ...
    def _detect_alerts(self, data: dict[str, Any],
                       current_time: float) -> None:
        ...
        if (self._last_rpm is not None and rpm is not None
                and abs(rpm - self._last_rpm) < 1.0  # RPM unchanged
                and tps > FROZEN_TPS_THRESHOLD  # Engine should be running
                and current_time - self._last_rpm_time
    ...
    def _add_alert(self, alert: Alert) -> None:
        ...
        for existing in self.alerts:
            if (existing.type == alert.type
                    and existing.channel == alert.channel
                    and alert.timestamp - existing.timestamp < 5.0):
    ...
    def get_coverage_stats(self) -> dict[str, Any]:
        ...
        if self._active_cell and self._active_cell in self.coverage_map:
            ...
            active_cell_info = {
                **cell.to_dict(),
                "afr_error_mean":
                ve_cell.afr_error_mean if ve_cell else None,
            }
    ...
    def get_ve_delta_stats(self) -> dict[str, Any]:
        ...
        total_error = sum(cell.afr_error_sum
                          for cell in self.ve_delta_map.values())
        total_count = sum(cell.afr_error_count
                          for cell in self.ve_delta_map.values())
