...
from System.Windows.Forms import (
	Form, Label, TextBox, ComboBox, CheckBox, Button,
...
def run_cli(values):
	...
	args = [
		"-m", "tools.synthetic.winpep8_cli", "peaks",
		"--run-id", values["run_id"],
		"--family", values["family"],
		"--displacement-ci", str(values["displacement_ci"]),
		"--max-hp", str(values["max_hp"]),
		"--hp-peak-rpm", str(values["hp_peak_rpm"]),
		"--max-tq", str(values["max_tq"]),
		"--tq-peak-rpm", str(values["tq_peak_rpm"]),
		"--rpm-points", str(values["rpm_points"]),
	]
...
class BridgeDialog(Form):
	...
	def _build_form(self, defaults):
		...
		self.tb_run_id = self._add_textbox(input_x, y, input_w,
			defaults.get("run_id", ""))
		y += row_h
		...
		self.tb_displacement = self._add_textbox(input_x, y, input_w,
			defaults.get("displacement_ci", DEFAULT_DISPLACEMENT))
		y += row_h

		# Peak HP
		self._add_label("Peak HP:", 15, y, label_w)
		self.tb_max_hp = self._add_textbox(input_x, y, input_w,
			defaults.get("max_hp", ""))
		y += row_h

		# HP Peak RPM
		self._add_label("HP Peak RPM:", 15, y, label_w)
		self.tb_hp_peak_rpm = self._add_textbox(input_x, y, input_w,
			defaults.get("hp_peak_rpm_default", ""))
		y += row_h

		# Peak TQ
		self._add_label("Peak TQ (lb-ft):", 15, y, label_w)
		self.tb_max_tq = self._add_textbox(input_x, y, input_w,
			defaults.get("max_tq", ""))
		y += row_h

		# TQ Peak RPM
		self._add_label("TQ Peak RPM:", 15, y, label_w)
		self.tb_tq_peak_rpm = self._add_textbox(input_x, y, input_w,
			defaults.get("tq_peak_rpm_default", ""))
		y += row_h

		# RPM Points
		self._add_label("RPM Points:", 15, y, label_w)
		self.tb_rpm_points = self._add_textbox(input_x, y, input_w,
			defaults.get("rpm_points", DEFAULT_RPM_POINTS))
		y += row_h
	...
	def _validate_and_parse(self):
		...
		if (len(normalized) > 1 and normalized[1] == ":") or normalized.startswith("/"):
			self._err("Run ID must be relative (e.g. test/powercore_smoke), not a full path.")
			return None
		...
		family = str(self.cb_family.SelectedItem) if self.cb_family.SelectedItem \
			else DEFAULT_FAMILY
