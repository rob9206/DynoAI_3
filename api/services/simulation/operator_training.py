...
class OperatorTrainingSimulator:
    ...
    def _update_thermal(self, dt: float):
        ...
        if self.training_state.egt_front < target_egt_front:
            self.training_state.egt_front += (target_egt_front - self.training_state.egt_front) * heat_rate
        else:
            self.training_state.egt_front += (target_egt_front - self.training_state.egt_front) * cool_rate
            
        if self.training_state.egt_rear < target_egt_rear:
            self.training_state.egt_rear += (target_egt_rear - self.training_state.egt_rear) * heat_rate
        else:
            self.training_state.egt_rear += (target_egt_rear - self.training_state.egt_rear) * cool_rate
            
        # Oil temperature
        target_oil = 180 + (tps / 100.0) * 40 + (rpm / 7000.0) * 20
        self.training_state.oil_temp += (target_oil - self.training_state.oil_temp) * 0.001
        
        # Coolant temperature  
        target_coolant = 185 + (tps / 100.0) * 15
        self.training_state.coolant_temp += (target_coolant - self.training_state.coolant_temp) * 0.002
