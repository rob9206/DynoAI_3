...
class LiveCaptureQueueStats:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "persist_lag_ms": round(self.persist_lag_ms, 2),
            "enqueue_rate_hz": round(self.samples_enqueued / max(time.time() - self.last_flush_time, 1.0), 2) if self.last_flush_time > 0 else 0,
        }
...
class LiveCaptureQueueManager:
    ...
    def _flush_aggregation_window(self) -> None:
        ...
        try:
            ...
            aggregated = self.adapter.aggregate_samples(
                self._sample_buffer,
                time_window_ms=AGGREGATION_WINDOW_MS
            )
            ...
            for point in aggregated:
                ...
                if self._realtime_engine is not None:
                    try:
                        ...
                        logger.warning(f"Realtime analysis error (non-blocking): {e}")
                
                item_id = self.queue.enqueue(
                    source="jetdrive_live",
                    data=point_dict,
                    priority=QueuePriority.HIGH,  # Real-time data is high priority
                    metadata={
    ...
    def _open_csv(self, path: Path) -> None:
        ...
        with self._csv_lock:
            try:
                ...
                columns = [
                    'timestamp_ms', 'rpm', 'horsepower', 'torque',
                    'afr', 'afr_front', 'afr_rear',
                    'map_kpa', 'tps', 'iat', 'ect',
                    'force_lbs', 'acceleration',
                    'speed_mph', 'gear',
                ]
    ...
    def enable_realtime_analysis(self, target_afr: float = 14.7) -> None:
        ...
        if self._realtime_engine is None:
            ...
            logger.info(f"Realtime analysis enabled (target AFR: {target_afr})")
        else:
