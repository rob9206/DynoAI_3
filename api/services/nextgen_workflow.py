...
from dynoai.core.log_normalizer import get_channel_readiness, normalize_dataframe
...
class TestPlannerConstraints:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "TestPlannerConstraints":
        return cls(
            ...
            preferred_test_environment=data.get("preferred_test_environment",
                                                "both"),
        )
...
def get_planner_constraints(
        vehicle_id: str = "default") -> TestPlannerConstraints:
...
def save_planner_constraints(constraints: TestPlannerConstraints,
                             vehicle_id: str = "default") -> bool:
...
class NextGenWorkflow:
    ...
    def _execute_pipeline(
        ...
        if not norm_result.presence.has_required:
            raise ValueError(
                "Required columns (rpm, map_kpa) not found in data")
        ...
        logger.info(
            f"Found {len(spark_valley_findings)} spark valley findings")
        ...
        logger.info(
            f"Generated {len(cause_tree_result.hypotheses)} hypotheses")
    ...
    def _build_summary(self, payload_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "mode_distribution": {
                "wot_percent":
                round(
                    mode_counts.get("wot", 0) / max(total_samples, 1) * 100,
                    1),
                "cruise_percent":
                round(
                    mode_counts.get("cruise", 0) / max(total_samples, 1) * 100,
                    1),
                "idle_percent":
                round(
                    mode_counts.get("idle", 0) / max(total_samples, 1) * 100,
                    1),
                "tip_in_percent":
                round(
                    mode_counts.get("tip_in", 0) / max(total_samples, 1) * 100,
                    1),
            },
