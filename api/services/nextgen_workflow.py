...
def save_planner_constraints(
    constraints: TestPlannerConstraints, vehicle_id: str = "default"
) -> bool:
...
class NextGenWorkflow:
    ...
    def _build_summary(self, payload_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        high_impact_gaps = sum(
            1
            for g in coverage_gaps_detailed
        ...
        return {
            ...
            "mode_distribution": {
                "wot_percent": round(
                    mode_counts.get("wot", 0) / max(total_samples, 1) * 100, 1
                ),
                "cruise_percent": round(
                    mode_counts.get("cruise", 0) / max(total_samples, 1) * 100, 1
                ),
                "idle_percent": round(
                    mode_counts.get("idle", 0) / max(total_samples, 1) * 100, 1
                ),
                "tip_in_percent": round(
                    mode_counts.get("tip_in", 0) / max(total_samples, 1) * 100, 1
                ),
            },
