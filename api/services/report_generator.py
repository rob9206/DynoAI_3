...
class DynoReportGenerator:
    ...
    def build_comparison_elements(
        ...
        elements.append(Paragraph(self.branding.shop_name, self.styles['ReportTitle']))
        elements.append(Paragraph("Run Comparison Report", self.styles['SectionHeader']))
        elements.append(Spacer(1, 6))
        elements.append(HRFlowable(
            width="100%",
            thickness=2,
        ...
        info_table = Table(info_data, colWidths=[1.0 * inch, 2.5 * inch, 1.0 * inch, 2.0 * inch])
        info_table.setStyle(TableStyle([
            ('FONTNAME', (0, 0), (0, -1), 'Helvetica-Bold'),
            ('FONTNAME', (2, 0), (2, -1), 'Helvetica-Bold'),
            ('FONTSIZE', (0, 0), (-1, -1), 10),
    ...
    def _build_comparison_peak_table(self, data: ComparisonReportData) -> list:
        ...
        table_data = [
            ...
            ['Peak HP', f"{pa.get('peak_hp', 0):.1f}", f"{pb.get('peak_hp', 0):.1f}",
             f"{deltas.get('peak_hp_gain', 0):+.1f}", f"{deltas.get('pct_change_hp', 0):+.1f}%"],
            ['Peak HP RPM', f"{pa.get('peak_hp_rpm', 0):.0f}", f"{pb.get('peak_hp_rpm', 0):.0f}", '-', '-'],
            ['Peak TQ', f"{pa.get('peak_tq', 0):.1f}", f"{pb.get('peak_tq', 0):.1f}",
             f"{deltas.get('peak_tq_gain', 0):+.1f}", f"{deltas.get('pct_change_tq', 0):+.1f}%"],
            ['Peak TQ RPM', f"{pa.get('peak_tq_rpm', 0):.0f}", f"{pb.get('peak_tq_rpm', 0):.0f}", '-', '-'],
        ]
        table = Table(table_data, colWidths=[1.5 * inch, 1.0 * inch, 1.0 * inch, 1.0 * inch, 1.0 * inch])
    ...
    def _build_comparison_power_overlay(self, data: ComparisonReportData) -> list:
        ...
        axes[0].plot(rpms, hp_a, '-', linewidth=2.2, color=self.branding.primary_color, label=f"{data.run_a_id} HP")
        axes[0].plot(rpms, hp_b, '--', linewidth=2.2, color=self.branding.accent_color, label=f"{data.run_b_id} HP")
        axes[0].set_ylabel('HP', fontsize=10, fontweight='bold')
        axes[0].grid(True, alpha=0.25)
        axes[0].legend(loc='upper left', fontsize=8)

        axes[1].plot(rpms, tq_a, '-', linewidth=2.2, color=self.branding.primary_color, label=f"{data.run_a_id} TQ")
        axes[1].plot(rpms, tq_b, '--', linewidth=2.2, color=self.branding.accent_color, label=f"{data.run_b_id} TQ")
        ...
        plt.tight_layout()
        img_buffer = io.BytesIO()
        fig.savefig(img_buffer, format='png', bbox_inches='tight', facecolor='white', edgecolor='none')
        plt.close(fig)
        img_buffer.seek(0)
    ...
    def _build_comparison_delta_chart(self, data: ComparisonReportData) -> list:
        ...
        elements: list[Any] = []
        elements.append(Paragraph("Delta by RPM (Run B - Run A)", self.styles['SectionHeader']))

        rpms = [p.get('rpm', 0) for p in data.curve_a]
        hp_delta = [b.get('hp', 0) - a.get('hp', 0) for a, b in zip(data.curve_a, data.curve_b)]
        tq_delta = [b.get('tq', 0) - a.get('tq', 0) for a, b in zip(data.curve_a, data.curve_b)]

        fig, ax = plt.subplots(figsize=(7, 3.4), dpi=150)
        fig.patch.set_facecolor('white')
        ax.axhline(0, color='#6B7280', linewidth=1.0)
        ax.plot(rpms, hp_delta, '-', color=self.branding.primary_color, linewidth=2.0, label='HP Delta')
        ax.plot(rpms, tq_delta, '-', color=self.branding.accent_color, linewidth=2.0, label='TQ Delta')
    ...
    def _build_comparison_afr_overlay(self, data: ComparisonReportData) -> list:
        ...
        fig.patch.set_facecolor('white')
        ax.plot(rpms, afr_f_a, '-', linewidth=2.0, color=self.branding.primary_color, label=f'{data.run_a_id} AFR F')
        ax.plot(rpms, afr_r_a, '-', linewidth=1.8, color=self.branding.primary_color, alpha=0.6, label=f'{data.run_a_id} AFR R')
        ax.plot(rpms, afr_f_b, '--', linewidth=2.0, color=self.branding.accent_color, label=f'{data.run_b_id} AFR F')
        ax.plot(rpms, afr_r_b, '--', linewidth=1.8, color=self.branding.accent_color, alpha=0.6, label=f'{data.run_b_id} AFR R')
        ax.set_xlabel('RPM', fontsize=10, fontweight='bold')
...
def _load_comparison_run(
    ...
    if afr_f_col in df.columns or afr_r_col in df.columns:
        afr_df = df[[rpm_col] + [c for c in [afr_f_col, afr_r_col] if c in df.columns]].copy()
            ...
            for rpm in DynoReportGenerator.RPM_BINS:
                ...
                if float(rpm) in afr_grouped.index:
                    if "afr_front" in afr_grouped.columns:
                        row["afr_front"] = float(afr_grouped.at[float(rpm), "afr_front"])
...
def _comparison_deltas(peaks_a: dict[str, float], peaks_b: dict[str, float]) -> dict[str, float]:
