...
class InnovateClient:
    ...
    def _load_calibration(self, calibration_file: str | None = None) -> dict:
        ...
        if not calibration_file.exists():
            logger.info(
                f"Calibration file not found: {calibration_file}. Using defaults (no offsets)."
            )
            return default_cal
        ...
        try:
            ...
            if self.serial_conn.in_waiting > 0:
                ...
                try:
                    ...
                    time.sleep(0.2)
                    if self.serial_conn.in_waiting > 0:
                        initial_data = self.serial_conn.read(
                            self.serial_conn.in_waiting
                        )
                        logger.info(
                            f"Device responding to G command: {len(initial_data)} bytes"
                        )
                        self._mts_buffer.extend(initial_data)
                except Exception:
                    pass
    ...
    def _extract_mts_packet_all_channels(
        ...
        try:
            ...
            for port in ports:
                ...
                if any(keyword in port_str for keyword in innovate_keywords):
                    logger.info(
                        f"Auto-detected potential Innovate device: {port.device}"
                    )
                    return port.device
