...
class CumulativeCoverage:
    ...
    aggregated_hit_count: dict[str,
                               list[list[int]]] = field(default_factory=dict)
...
def save_cumulative_coverage(coverage: CumulativeCoverage) -> bool:
    ...
    try:
        ...
        logger.info(
            f"Saved coverage tracker for {coverage.vehicle_id} ({coverage.total_runs} runs)"
        )
        return True
    except Exception as e:
...
def aggregate_run_coverage(
    ...
    for surface_id, surface_dict in surfaces.items():
        ...
        if surface_id not in coverage.aggregated_hit_count:
            ...
            coverage.aggregated_hit_count[surface_id] = [[0] * cols
                                                         for _ in range(rows)]
    ...
    logger.info(f"Aggregated coverage for {vehicle_id}: run {run_id} "
...
def get_cumulative_gaps(
    ...
    regions = [
        {
            ...
            "high",
            "description":
            "High-load midrange - knock-sensitive and torque peak region",
        },
        {
            "name":
            "idle_low_map",
            ...
            "medium",
            "description":
            "Idle and low-load - stability and sensor quality critical",
        },
        {
            "name": "tip_in_zone",
            "rpm_range": (2000, 4500),
            ...
            "impact": "high",
            "description":
            "Tip-in transition zone - transient fueling sensitive",
        },
    ]
        ...
        for region in regions:
            ...
            for i in range(rows):
                for j in range(cols):
                    ...
                    if (rpm_min <= rpm_estimate <= rpm_max
                            and map_min <= map_estimate <= map_max):
                        total_cells += 1
    ...
    gaps.sort(
        key=lambda g: (impact_order.get(g["impact"], 3), -g["empty_cells"]))
