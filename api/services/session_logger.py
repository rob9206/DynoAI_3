...
class SessionLogger:
    ...
    def record_baseline(
        self,
        ve_path: Path,
        description: str = "Initial VE table baseline"
    ) -> TimelineEvent:
        """
    ...
    def record_rollback(
        ...
        event = TimelineEvent(
            ...
            metadata={
                ...
                "original_apply_at": rollback_info.get("original_apply_metadata", {}).get("applied_at_utc")
            }
        )
    ...
    def compute_diff(
        ...
        return {
            ...
            "summary": {
                ...
                "avg_change": round(sum(non_zero) / len(non_zero), 4) if non_zero else 0,
                "max_change": round(max(non_zero), 4) if non_zero else 0,
                "min_change": round(min(non_zero), 4) if non_zero else 0
            },
            "changes": sorted(changes, key=lambda c: abs(c["delta"]), reverse=True)[:20]  # Top 20 changes
        }
