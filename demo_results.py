...
print("\n[PHASE 1: MODE DETECTION]")
print(
    f"  TIP_OUT: {ms['tip_out']:4d} samples ({ms['tip_out'] / total * 100:5.1f}%)"
)
print(
    f"  TIP_IN:  {ms['tip_in']:4d} samples ({ms['tip_in'] / total * 100:5.1f}%)"
)
print(
    f"  CRUISE:  {ms['cruise']:4d} samples ({ms['cruise'] / total * 100:5.1f}%)"
)
...
if steps:
    ...
    for i, test in enumerate(steps[:3], 1):
        ...
        print(
            f"        RPM: {rpm[0]}-{rpm[1]}, Load: {map_kpa[0]}-{map_kpa[1]} kPa"
        )
