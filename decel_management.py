...
def detect_decel_events(
    ...
    for i in range(len(records)):
        ...
        if not in_event:
            ...
            is_steady_closed = (tps_val <= cfg['tps_max_at_end']) and (rpm_drop > 5.0) # Significant drop per sample
            
            if (is_rapid_close or is_steady_closed) and (cfg['rpm_min'] <= rpm_float <= cfg['rpm_max']):
                in_event = True
                event_start = i
            ...
            if tps_rose or rpm_too_low or i == len(records) - 1:
                ...
                if (cfg['duration_min_ms'] <= duration_ms <= cfg['duration_max_ms']):
                    ...
                    if start_rpm is not None:
                        ...
                        avg_rate = sum(tps_rate[event_start:i]) / max(1, i - event_start)
...
def write_decel_overlay_csv(
    ...
    with open(output_path, 'w', newline='', encoding='utf-8') as f:
        ...
        # Data rows
        for i, rpm in enumerate(rpm_bins):
            row = [str(rpm)] + [f'{overlay[i][j]*100:+.2f}' for j in range(len(kpa_bins))]
            writer.writerow(row)
...
def generate_decel_report(
    ...
    decel_cells = [(i, j) for i in range(len(overlay)) for j in range(len(overlay[0]))
                   if overlay[i][j] > 0]
    if decel_cells:
    ...
    # Add tradeoffs
    fuel_impact = "-0.5 to -1.0 MPG" if severity == DecelSeverity.MEDIUM else \
                  "-0.3 to -0.5 MPG" if severity == DecelSeverity.LOW else "-1.0 to -2.0 MPG"
    report.tradeoffs = {
