"""Raise hot-side accel enrichment to fix hesitation on initial throttle.

Use when previous reduction made tip-in too lean.
Keeps cold-side untouched.
"""
...
def raise_accel_cells(
    temps: list[float],
    current: list[float],
    hot_threshold: float = 140.0,
    target_hot: float = 0.40,
...
def run(args: argparse.Namespace) -> int:
    ...
    new_vals, notes = raise_accel_cells(temps, current, args.hot_threshold, args.target_hot)
    ...
    print("  - Hot-side tip-in enrichment raised -> should fix hesitation on initial throttle.")
    print("  - Cold-start behavior unchanged.")
    print("  - Idle VE reduction and decel fix preserved.")
    return 0
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    p.add_argument("--hot-threshold", type=float, default=140.0)
    p.add_argument("--target-hot", type=float, default=0.40)
